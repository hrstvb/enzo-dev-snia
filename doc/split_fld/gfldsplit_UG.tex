...
local thermodynamic equilibrium (no chemical ionization), an internal
model for Hydrogen ionization, or through using Enzo's built-in
chemical ionization and gas heating mechanisms.
...
correction + chemical evolution system, using an operator-split
...
solver possible for these interacting processes.  This approach also
allows the user to choose between different chemistry and cooling
solvers, depending on the type of chemistry computations desired.
...
\item Multi-frequency radiation -- This solver evolves a single radiation
  field, either monochromatic or frequency-integrated.  As such, it
  does not currently allow for selective absorption of different
  radiation frequencies by different chemical fields.  We are
  currently working on a multi-frequency version of the 
  {\tt gFLDSplit} module, to enable more realistic simulation of such
  physical scenarios. 
\end{itemize}
...
\item {\tt RadiativeCooling} [0] -- this is the input parameter that
  chooses between the {\tt gFLDSplit}-specific Hydrogen chemistry/heating
  solver and Enzo's built-in chemistry/heating solvers.
  \begin{itemize}
  \item 0 $\Rightarrow$ use the {\tt gFLDSplit} chemistry/cooling
    solver (as described in this document).  We note that this
    currently handles primordial Hydrogen chemistry only (HI, HII).
  \item 1 $\Rightarrow$ use Enzo's built-in chemistry/cooling
    solvers.  A nonzero value here disables {\tt gFLDSplit} from
    evolving {\em either} the gas energy or chemistry; however, it
    only turns on Enzo's cooling solver.  
  \end{itemize}
\item {\tt Multispecies} [0] -- when {\tt RadiativeCooling} is set to
  ...
  documentation, 
\item {\tt RadiativeTransferHydrogenOnly} [0] -- when 
  {\tt Multispecies} is set to 1, a nonzero value for \\
  {\tt RadiativeTransferHydrogenOnly} tells Enzo's chemistry modules
  to evolve only Hydrogen chemistry (no Helium).  Therefore, use of
  {\tt RadiativeCooling}=1, {\tt Multispecies}=1 and \\
  {\tt RadiativeTransferHydrogenOnly}=1 forces Enzo's chemistry
  solvers to emulate the chemistry/cooling solvers built into
  {\tt gFLDSplit}.
\end{itemize}
...
grey radiation field.  If this new {\tt ProblemType} should allow 
{\tt gFLDSplit} to work together with Enzo's existing
chemistry/heating solvers, additional baryon fields must be allocated
...
this document.  We note that all of the following chemistry-related
parameters only affect the ionization/heating solvers built into {\tt
  gFLDSplit}; if other solvers are used instead, these parameters will
be ignored.
\begin{itemize}
