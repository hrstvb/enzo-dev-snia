...
int AutomaticSubgridGeneration(parmstruct *Parameters)
{
  ...
  for (level=0; level <= Parameters->MaximumInitialRefinementLevel; level++) {
	...
	StartIndex[dim] = max(int(LevelDims[dim]*Parameters->RefineRegionLeftEdge[dim])-Parameters->SubgridBuffer, 0);
	EndIndex[dim] = int(LevelDims[dim]*Parameters->RefineRegionRightEdge[dim])+Parameters->SubgridBuffer;
