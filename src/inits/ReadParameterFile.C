...
int ReadParameterFile(FILE *fptr, parmstruct *Parameters)
{
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) {
    ...
    ret += sscanf(line, "TopGridStart = %"ISYM" %"ISYM" %"ISYM,
		  Parameters->TopGridStart,
		  Parameters->TopGridStart+1,
		  Parameters->TopGridStart+2);
    ret += sscanf(line, "TopGridEnd = %"ISYM" %"ISYM" %"ISYM,
		  Parameters->TopGridEnd,
		  Parameters->TopGridEnd+1,
		  Parameters->TopGridEnd+2);
    ...
    ret += sscanf(line, "SubgridBuffer = %"ISYM, &Parameters->SubgridBuffer);
  ...
  for (dim = 0; dim < Parameters->Rank; dim++) {
    printf("Check: Parameters->MaxDims[%"ISYM"] = %"ISYM"\n", dim, Parameters->MaxDims[dim]); 
    printf("Check: Parameters->NewCenterFloat[%"ISYM"] = %"FSYM"\n", dim, Parameters->NewCenterFloat[dim]);
    printf("Check: Parameters->NewCenter[%"ISYM"] = %"ISYM"\n", dim, Parameters->NewCenter[dim]);
    printf("Check: Parameters->StartIndexInNewCenterTopGridSystem[%"ISYM"] = %"ISYM"\n", dim, Parameters->StartIndexInNewCenterTopGridSystem[dim]);
    printf("Check: Parameters->EndIndexInNewCenterTopGridSystem[%"ISYM"] = %"ISYM"\n", dim, Parameters->EndIndexInNewCenterTopGridSystem[dim]);
  ...
  for (dim = 0; dim < Parameters->Rank; dim++) {
    printf("Parameters->StartIndexInNewCenterTopGridSystem[%"ISYM"] = %"ISYM"\n", dim, Parameters->StartIndexInNewCenterTopGridSystem[dim]);
    printf("Parameters->EndIndexInNewCenterTopGridSystem[%"ISYM"] = %"ISYM"\n", dim, Parameters->EndIndexInNewCenterTopGridSystem[dim]);
  }
...
// grid to the ring code and ultimately to enzo.
  ...
  for (dim = Parameters->Rank; dim < 3; dim++)
  {
    ...
    printf("TopGridStart: %"ISYM" %"ISYM" %"ISYM"\n", Parameters->TopGridStart[0], Parameters->TopGridStart[1], Parameters->TopGridStart[2]);
    printf("TopGridEnd:   %"ISYM" %"ISYM" %"ISYM"\n", Parameters->TopGridEnd[0], Parameters->TopGridEnd[1], Parameters->TopGridEnd[2]);
