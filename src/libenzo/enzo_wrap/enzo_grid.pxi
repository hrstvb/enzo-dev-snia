...
cdef class grid:
    ...
    def get_fluxes(self, int index):
        ...
        for dim in range(ndims):
            ...
            if g.BoundaryFluxes.LeftFluxes[index][dim] == NULL:
                f1 = None
            else:
                f1 = cnp.PyArray_SimpleNewFromData(
                            fdims, &size, E_ENPY_BFLOAT,
                            g.BoundaryFluxes.LeftFluxes[index][dim])
            if g.BoundaryFluxes.RightFluxes[index][dim] == NULL:
                f2 = None
            else:
                f2 = cnp.PyArray_SimpleNewFromData(
                            fdims, &size, E_ENPY_BFLOAT,
                            g.BoundaryFluxes.RightFluxes[index][dim])
            fluxes.append((f1,f2))
        return fluxes
    ...
    property PotentialField:
        def __get__(self):
            if self.thisptr.PotentialField == NULL: return None
            cdef int dim
            cdef Eint32 ndims = self.thisptr.GridRank
            cdef np.npy_intp dims[MAX_DIMENSION]
            cdef np.ndarray f
            for dim in range(ndims):
                dims[dim] = self.thisptr.GravitatingMassFieldDimension[dim]
            f = cnp.PyArray_SimpleNewFromData(ndims, dims, E_ENPY_BFLOAT,
                                self.thisptr.PotentialField)
            return f

    property AccelerationField:
        def __get__(self):
            cdef int dim, size = 1
            cdef Eint32 ndims = self.thisptr.GridRank
            cdef np.npy_intp dims[3]
            ...
            cdef Eflt *bfield 
            for dim in range(ndims):
                bfield = self.thisptr.AccelerationField[dim]
                if bfield == NULL:
                    pos.append(None)
                else:
                    pos.append(cnp.PyArray_SimpleNewFromData(
    ...
    # Now the cell-level quantities, which strictly speaking
    # we won't need to store
    ...
    property CellWidth:
        def __get__(self):
            ...
            pos = []
            for dim in range(ndims):
                dims = self.thisptr.GridDimension[dim]
                print ndims, dims
                pos.append(cnp.PyArray_SimpleNewFromData(
                            1, &dims, E_ENPY_PFLOAT,
                            self.thisptr.CellWidth[dim]))
            # Might need to set field flags here
            return pos

    property CellLeftEdge:
        def __get__(self):
            cdef int dim
            cdef Eint32 ndims = self.thisptr.GridRank
            cdef np.npy_intp dims
...
#    '*ParticlePosition[MAX_DIMENSION]'
#    '*ParticleVelocity[MAX_DIMENSION]'
#    '*ParticleAcceleration[MAX_DIMENSION+1]'
#    '*ParticleMass'
#    '*ParticleNumber'
#    '*ParticleType'
#    '*ParticleAttribute[MAX_NUMBER_OF_PARTICLE_ATTRIBUTES]'
#    '*PotentialField'
#    '*AccelerationField[MAX_DIMENSION]'
#    '*CellLeftEdge[MAX_DIMENSION]'
#    '*CellWidth[MAX_DIMENSION]'

# SKIPPED and not dealt with:

#    '*InterpolatedField[MAX_NUMBER_OF_BARYON_FIELDS]'
#    '*RandomForcingField[MAX_DIMENSION]'
#    '*divB'
#    '*gradPhi[MAX_DIMENSION]'
#    '*GravitatingMassField'
#    '*GravitatingMassFieldParticles'
#    '*FlaggingField'
#    '*MassFlaggingField'
#    '*ParticleMassFlaggingField'
