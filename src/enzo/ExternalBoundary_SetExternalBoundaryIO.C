...
int READ_BT(boundary_type *bt_buffer, int field, int dim, int face, int slabsize, int BoundaryDimension[],
int BoundaryRank, int Nfields);
int READ_BV(float *bv_buffer, int field, int dim, int face, int slabsize, int BoundaryDimension[], int BoundaryRank,
int Nfields);
...
int ExternalBoundary::SetExternalBoundary(int FieldRank, int GridDims[], int GridOffset[], int StartIndex[],
	int EndIndex[], float *Field, int FieldType)
{
	...
	for(field = 0; field < NumberOfBaryonFields; field++)
		if(FieldType == BoundaryFieldType[field])
			break;
	if(field == NumberOfBaryonFields)
	{
		ENZO_VFAIL("Field type (%"ISYM") not found in Boundary(%lld).\n", FieldType, NumberOfBaryonFields);
	}
...
//          bt_buffer[bindex] = 3;
            ...
            *index = *(index + (EndIndex[0] - StartIndex[0] + 1));
					...
					default:
						...
						fprintf(stderr,
								"field %"ISYM" dim %"ISYM" face %"ISYM" slab %"ISYM" bindex %"ISYM" btb %"ISYM"\n",
		...
		for(i = 0; i < GridDims[0] - EndIndex[0] - 1; i++)
			for(j = 0; j < GridDims[1]; j++)
				for(k = 0; k < GridDims[2]; k++)
				{
					...
					index = Field + i + EndIndex[0] + 1 + j * GridDims[0] + k * GridDims[1] * GridDims[0];
					bindex = j + GridOffset[1] + (k + GridOffset[2]) * BoundaryDimension[1];
		...
		for(j = 0; j < GridDims[1] - EndIndex[1] - 1; j++)
			for(i = 0; i < GridDims[0]; i++)
				for(k = 0; k < GridDims[2]; k++)
				{
					...
					index = Field + i + (j + EndIndex[1] + 1) * GridDims[0] + k * GridDims[1] * GridDims[0];
					bindex = i + GridOffset[0] + (k + GridOffset[2]) * BoundaryDimension[0];
		...
		for(k = 0; k < GridDims[2] - EndIndex[2] - 1; k++)
			for(i = 0; i < GridDims[0]; i++)
				for(j = 0; j < GridDims[1]; j++)
				{
					...
					index = Field + i + j * GridDims[0] + (k + EndIndex[2] + 1) * GridDims[1] * GridDims[0];
					bindex = i + GridOffset[0] + (j + GridOffset[1]) * BoundaryDimension[0];
