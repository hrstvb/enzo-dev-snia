...
!
!=======================================================================
subroutine MFSplit_RadiationSource(eta1, eta2, eta3, NGamDot_FS, time,  &
     a, Model, ProbType, ESpectrum, Nchem, SMEmiss, NGammaDot,          &
     chiint, chinuint, EtaRadius, EtaCenter, aUn, dUn, vUn, lUn, tUn,   &
     E1Un, E2Un, E3Un, eUn, nUn, Nx, Ny, Nz, NGxl, NGxr, NGyl, NGyr,    &
...
!     chiint     - int_nu0^infty chi(nu) dnu
!     chinuint   - int_0^infty chi(nu) nu0/nu dnu
...
!           SMEmiss is disabled.
!
!     Note: the emissivity sources should be scaled to normalized 
!           radiation units using the factors E1Un, E2Un and E3Un.  
!
!  OUTPUT ARGUMENTS: 
...
#include "fortran.def"
  ...
  real, intent(in) :: EtaCenter(3), chiint, chinuint
  real, intent(in) :: aUn, dUn, vUn, lUn, tUn, E1Un, E2Un, E3Un, eUn, nUn
  ...
  call MFSplit_RadiationSpectrum(chi1, h_nu1/hp, ESpectrum, 1)
  call MFSplit_RadiationSpectrum(chi2, h_nu2/hp, ESpectrum, 1)
  call MFSplit_RadiationSpectrum(chi3, h_nu3/hp, ESpectrum, 1)

  ! compute the effective NGammaDot for the free-streaming emissivity
  NGamDot_FS = NGammaDot*chiint/chinuint
  ...
  if (ProbType == 460) then
     ...
     if (EtaRadius == 0.d0) then
        ...
        etaconst1 = h_nu1*NGammaDot*chi1/chinuint/dV/E1Un
        etaconst2 = h_nu1*NGammaDot*chi2/chinuint/dV/E2Un
        etaconst3 = h_nu1*NGammaDot*chi3/chinuint/dV/E3Un
        ...
        ! compute eta factor for given ionization source
        etaconst1 = h_nu1*NGammaDot*chi1/chinuint/dV/8.d0/(EtaRadius**3)/E1Un
        etaconst2 = h_nu1*NGammaDot*chi2/chinuint/dV/8.d0/(EtaRadius**3)/E2Un
        etaconst3 = h_nu1*NGammaDot*chi3/chinuint/dV/8.d0/(EtaRadius**3)/E3Un
...
!!$     print *,'  max(eta3) =',maxval(eta3)
  ...
  !   emissivity flux along x=0 wall (NGammaDot photons/s/cm^2)
     ...
     if (x0L == 0.d0) then
        ...
        eta1(1,:,:) = h_nu1*NGammaDot*chi1/chinuint/dy/E1Un
        eta2(1,:,:) = h_nu1*NGammaDot*chi2/chinuint/dy/E2Un
        eta3(1,:,:) = h_nu1*NGammaDot*chi3/chinuint/dy/E3Un
  ...
  elseif (ProbType == 414) then
     ...
     eta1(int(Nx/2),int(Ny/2),int(Nz/2)) = h_nu1*NGammaDot*chi1/chinuint/dV/E1Un
     eta2(int(Nx/2),int(Ny/2),int(Nz/2)) = h_nu1*NGammaDot*chi2/chinuint/dV/E2Un
     eta3(int(Nx/2),int(Ny/2),int(Nz/2)) = h_nu1*NGammaDot*chi3/chinuint/dV/E3Un
     
  !   homogeneous emissivity field w/ strength NGammaDot/dV
  elseif (ProbType == 462) then

     ! compute eta factor for given ionization source
     eta1 = h_nu1*NGammaDot*chi1/chinuint/dV/E1Un
     eta2 = h_nu1*NGammaDot*chi2/chinuint/dV/E2Un
     eta3 = h_nu1*NGammaDot*chi3/chinuint/dV/E3Un
        
  endif ! ProbType

  return
end subroutine MFSplit_RadiationSource
!=======================================================================
...
subroutine MFSplit_GasEnergySource(ecsrc, time, a, Model, ProbType,     &
     Nchem, aUn, dUn, vUn, lUn, tUn, E1Un, E2Un, E3Un, eUn, nUn, Nx,    &
...
!  date:       December 2009
!
!  PURPOSE: Empty shell routine, in case we need gas energy source 
!           terms in the future.
!=======================================================================
  implicit none
#include "fortran.def"
  ...
  return
end subroutine MFSplit_GasEnergySource
!=======================================================================
...
subroutine MFSplit_ChemistrySource(HIsrc, HeIsrc, HeIIsrc, time, a, Model, &
     ProbType, Nchem, aUn, dUn, vUn, lUn, tUn, E1Un, E2Un, E3Un, eUn, nUn, &
     Nx, Ny, Nz, NGxl, NGxr, NGyl, NGyr, NGzl, NGzr, x0L, x0R, x1L, x1R,   &
     x2L, x2R, ier)
!=======================================================================
!  written by: Daniel R. Reynolds
!  date:       December 2009
!
!  PURPOSE: Empty shell routine, in case we need chemistry source 
!           terms in the future.
!=======================================================================
  implicit none
#include "fortran.def"
  ...
  real, intent(in) :: x0L, x0R, x1L, x1R, x2L, x2R
  real, dimension(1-NGxl:Nx+NGxr,1-NGyl:Ny+NGyr,1-NGzl:Nz+NGzr), &
       intent(out) :: HIsrc, HeIsrc, HeIIsrc
  ...
  return
end subroutine MFSplit_ChemistrySource
!=======================================================================
