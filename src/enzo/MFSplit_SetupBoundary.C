...
int MFSplit::SetupBoundary(int Dim, int Face, int BdryConst, float *BdryData) 
{
//   if (debug)
//     printf("Entering MFSplit::SetupBoundary routine\n");
  ...
  // Error check
  if ((Dim < 0) || (Dim >= rank)) 
    ENZO_VFAIL("MFSplit SetupBoundary: DIM %"ISYM" out of bounds.\n",Dim)
  if ((Face != 0) && (Face != 1)) 
    ENZO_VFAIL("MFSplit SetupBoundary: Face %"ISYM" != {0,1}.\n",Face)
  ...
  if (EBdryVals[Dim][Face] != NULL)  delete[] EBdryVals[Dim][Face];
