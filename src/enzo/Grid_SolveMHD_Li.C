...
int grid::SolveMHD_Li(int CycleNumber, int NumberOfSubgrids, fluxes *SubgridFluxes[], float *CellWidthTemp[],
...
//  int hack = 0; //a flag for testing the solver.
	...
	this->IdentifyPhysicalQuantities(DensNum, GENum, Vel1Num, Vel2Num, Vel3Num, TENum, B1Num, B2Num, B3Num);
	...
	else
	{
		...
		if(EquationOfState == 0)
		{
			...
			{
				pressure[ii] = (Gamma - 1)
						* (BaryonField[TENum][ii]
								- 0.5
										* (BaryonField[Vel1Num][ii] * BaryonField[Vel1Num][ii]
												+ BaryonField[Vel2Num][ii] * BaryonField[Vel2Num][ii]
												+ BaryonField[Vel3Num][ii] * BaryonField[Vel3Num][ii])
										* BaryonField[DensNum][ii]
								- 0.5
										* (BaryonField[B1Num][ii] * BaryonField[B1Num][ii]
	...
	float * MagFluxX1 = Fluxes[0], *MagFluxX2 = Fluxes[0] + MagneticSize[0], *MagFluxY1 = Fluxes[1], *MagFluxY2 =
			Fluxes[1] + MagneticSize[1], *MagFluxZ1 = Fluxes[2], *MagFluxZ2 = Fluxes[2] + MagneticSize[2];
...
//	ClearOuterVelocities(pressure, NULL, MetaData, NULL, NULL, NULL);
			...
			for(kk = 0; kk < GridDimension[2]; kk++)
			{
				for(jj = 0; jj < GridDimension[1]; jj++)
				{
					for(ii = 0; ii < GridDimension[0]; ii++)
					{
						...
						field_line[ii + line_size * 1] = BaryonField[Vel1Num][index_bf]
								* BaryonField[DensNum][index_bf];
						field_line[ii + line_size * 2] = BaryonField[Vel2Num][index_bf]
								* BaryonField[DensNum][index_bf];
						field_line[ii + line_size * 3] = BaryonField[Vel3Num][index_bf]
								* BaryonField[DensNum][index_bf];
						...
						if(EquationOfState == 0)
						{
							...
							field_line[ii + line_size * 8] = pressure[index_bf]
									/ POW(BaryonField[DensNum][index_bf], Gamma - 1);
//              if(jj==55 && kk==55)
...
//        	  TRACEF("      xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx step, ii, jj, kk = * %lld %lld", jj, kk);
						...
						hack = 1;
					FORTRAN_NAME(pde1dsolver_mhd_new)(
							field_line, colour_line, &line_size, &nu, &startindex, &endindex,
							&NumberOfColours,
							CellWidthTemp[0],
							&dtFixed,
							...
							flux_colour, diffusion_line, &Gamma, &csmin, &rhomin, &MHDCTDualEnergyMethod,
							&MHDCTSlopeLimiter, &RiemannSolver, &ReconstructionMethod, &PPMDiffusionParameter,
							&MHDCTPowellSource, &Theta_Limiter, &CycleNumber, &GravityOn, gravity_line, a,
							&EquationOfState, &IsothermalSoundSpeed, &hack);

					for(ii = 0; ii < GridDimension[0]; ii++)
					{
						index_bf = ii + GridDimension[0] * (jj + GridDimension[1] * kk);
						BaryonField[DensNum][index_bf] = field_line[ii + line_size * 0];
						BaryonField[Vel1Num][index_bf] = field_line[ii + line_size * 1]
								/ field_line[ii + line_size * 0];
						BaryonField[Vel2Num][index_bf] = field_line[ii + line_size * 2]
								/ field_line[ii + line_size * 0];
						BaryonField[Vel3Num][index_bf] = field_line[ii + line_size * 3]
								/ field_line[ii + line_size * 0];
						...
						if(EquationOfState == 0)
						{
							...
							pressure[index_bf] = field_line[ii + line_size * 8]
									* POW(field_line[ii + line_size * 0], (Gamma - 1));
						}
						if(NumberOfColours > 0)
						{
							for(nColour = 0; nColour < NumberOfColours; nColour++)
					...
					for(subgrid = 0; subgrid < NumberOfSubgrids; subgrid++)
					{
								...
								&& (kk >= fjstart[dim][subgrid] && kk <= fjend[dim][subgrid]))
						{
							...
							SubgridFluxes[subgrid]->LeftFluxes[DensNum][dim][offset] = dtdx
									* flux_line[ii + line_size * 0];
							SubgridFluxes[subgrid]->LeftFluxes[Vel1Num][dim][offset] = dtdx
									* flux_line[ii + line_size * 1];
							SubgridFluxes[subgrid]->LeftFluxes[Vel2Num][dim][offset] = dtdx
									* flux_line[ii + line_size * 2];
							SubgridFluxes[subgrid]->LeftFluxes[Vel3Num][dim][offset] = dtdx
									* flux_line[ii + line_size * 3];
							if(EquationOfState == 0)
							{
								SubgridFluxes[subgrid]->LeftFluxes[TENum][dim][offset] = dtdx
										* flux_line[ii + line_size * 7];
							}
							if(DualEnergyFormalism)
							{
							...
							if(NumberOfColours > 0)
							{
								for(nColour = 0; nColour < NumberOfColours; nColour++)
								{
									SubgridFluxes[subgrid]->LeftFluxes[colnum[nColour]][dim][offset] = dtdx
											* flux_colour[ii + line_size * nColour];
								}
							}
							ii = rindex[dim][subgrid];
							SubgridFluxes[subgrid]->RightFluxes[DensNum][dim][offset] = dtdx
									* flux_line[ii + line_size * 0];
							SubgridFluxes[subgrid]->RightFluxes[Vel1Num][dim][offset] = dtdx
									* flux_line[ii + line_size * 1];
							SubgridFluxes[subgrid]->RightFluxes[Vel2Num][dim][offset] = dtdx
									* flux_line[ii + line_size * 2];
							SubgridFluxes[subgrid]->RightFluxes[Vel3Num][dim][offset] = dtdx
									* flux_line[ii + line_size * 3];
							if(EquationOfState == 0)
							{
								SubgridFluxes[subgrid]->RightFluxes[TENum][dim][offset] = dtdx
										* flux_line[ii + line_size * 7];
							}
							if(DualEnergyFormalism)
							{
							...
							if(NumberOfColours > 0)
							{
								for(nColour = 0; nColour < NumberOfColours; nColour++)
								{
									SubgridFluxes[subgrid]->RightFluxes[colnum[nColour]][dim][offset] = dtdx
											* flux_colour[ii + line_size * nColour];
								}
							}
							//end color fluxes
		...
		if((n % 3 == 1))
		{
			if(nyz == 1)
			{
				TransverseMagneticFlux(BaryonField[Vel2Num], BaryonField[Vel3Num], BaryonField[Vel1Num],
										BaryonField[B2Num], BaryonField[B3Num], BaryonField[B1Num], MagFluxY2,
										MagFluxY1, GridDimension, MagneticDims[1]);
				...
				for(kk = 0; kk < GridDimension[2]; kk++)
				{
					for(ii = 0; ii < GridDimension[0]; ii++)
					{
						for(jj = 0; jj < GridDimension[1]; jj++)
						{
							...
							field_line[jj + line_size * 1] = BaryonField[Vel2Num][index_bf]
									* BaryonField[DensNum][index_bf];
							field_line[jj + line_size * 2] = BaryonField[Vel3Num][index_bf]
									* BaryonField[DensNum][index_bf];
							field_line[jj + line_size * 3] = BaryonField[Vel1Num][index_bf]
									* BaryonField[DensNum][index_bf];
							field_line[jj + line_size * 4] = BaryonField[B2Num][index_bf];
							field_line[jj + line_size * 5] = BaryonField[B3Num][index_bf];
							field_line[jj + line_size * 6] = BaryonField[B1Num][index_bf];
							if(EquationOfState == 0)
							{
								field_line[jj + line_size * 7] = BaryonField[TENum][index_bf];
								field_line[jj + line_size * 8] = pressure[index_bf]
										/ POW(BaryonField[DensNum][index_bf], Gamma - 1);
							}
							for(nColour = 0; nColour < NumberOfColours; nColour++)
							{
...
//        	  TRACEF("      yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy step, ii, jj, kk = %lld * %lld", ii, kk);
						...
						for(jj = 0; jj < GridDimension[1]; jj++)
						{
							...
							BaryonField[Vel1Num][index_bf] = field_line[jj + line_size * 3]
									/ field_line[jj + line_size * 0];
							BaryonField[Vel2Num][index_bf] = field_line[jj + line_size * 1]
									/ field_line[jj + line_size * 0];
							BaryonField[Vel3Num][index_bf] = field_line[jj + line_size * 2]
									/ field_line[jj + line_size * 0];
							BaryonField[B1Num][index_bf] = field_line[jj + line_size * 6];
							BaryonField[B2Num][index_bf] = field_line[jj + line_size * 4];
							BaryonField[B3Num][index_bf] = field_line[jj + line_size * 5];
							if(EquationOfState == 0)
							{
								BaryonField[TENum][index_bf] = field_line[jj + line_size * 7];
								pressure[index_bf] = field_line[jj + line_size * 8]
										* POW(field_line[jj + line_size * 0], (Gamma - 1));
							}
							if(NumberOfColours > 0)
							{
								for(nColour = 0; nColour < NumberOfColours; nColour++)
						...
						for(subgrid = 0; subgrid < NumberOfSubgrids; subgrid++)
						{
							if((ii >= fistart[dim][subgrid] && ii <= fiend[dim][subgrid])
									&& (kk >= fjstart[dim][subgrid] && kk <= fjend[dim][subgrid]))
							{
								offset = (ii - fistart[dim][subgrid])
										+ (kk - fjstart[dim][subgrid]) * nfi[dim][subgrid];
								jj = lindex[dim][subgrid];
								SubgridFluxes[subgrid]->LeftFluxes[DensNum][dim][offset] = dtdx
										* flux_line[jj + line_size * 0];
								SubgridFluxes[subgrid]->LeftFluxes[Vel1Num][dim][offset] = dtdx
										* flux_line[jj + line_size * 3];
								SubgridFluxes[subgrid]->LeftFluxes[Vel2Num][dim][offset] = dtdx
										* flux_line[jj + line_size * 1];
								SubgridFluxes[subgrid]->LeftFluxes[Vel3Num][dim][offset] = dtdx
										* flux_line[jj + line_size * 2];
								if(EquationOfState == 0)
								{
									SubgridFluxes[subgrid]->LeftFluxes[TENum][dim][offset] = dtdx
											* flux_line[jj + line_size * 7];
								} //te flux
								if(DualEnergyFormalism)
								{
								...
								if(NumberOfColours > 0)
								{
									...
									{
										SubgridFluxes[subgrid]->LeftFluxes[colnum[nColour]][dim][offset] = dtdx
												* flux_colour[jj + line_size * nColour];
									}
								}
		...
		if((n % 3 == 2))
		{
			if(nzz == 1)
			{
				TransverseMagneticFlux(BaryonField[Vel3Num], BaryonField[Vel1Num], BaryonField[Vel2Num],
										BaryonField[B3Num], BaryonField[B1Num], BaryonField[B2Num], MagFluxZ1,
										MagFluxZ2, GridDimension, MagneticDims[2]);
				...
				{ //DO is this the fastest order?
					for(jj = 0; jj < GridDimension[1]; jj++)
					{
						for(kk = 0; kk < GridDimension[2]; kk++)
						{
							...
							field_line[kk + line_size * 1] = BaryonField[Vel3Num][index_bf]
									* BaryonField[DensNum][index_bf];
							field_line[kk + line_size * 2] = BaryonField[Vel1Num][index_bf]
									* BaryonField[DensNum][index_bf];
							field_line[kk + line_size * 3] = BaryonField[Vel2Num][index_bf]
									* BaryonField[DensNum][index_bf];
							field_line[kk + line_size * 4] = BaryonField[B3Num][index_bf];
							field_line[kk + line_size * 5] = BaryonField[B1Num][index_bf];
							field_line[kk + line_size * 6] = BaryonField[B2Num][index_bf];
							if(EquationOfState == 0)
							{
								field_line[kk + line_size * 7] = BaryonField[TENum][index_bf];
								field_line[kk + line_size * 8] = pressure[index_bf]
										/ POW(BaryonField[DensNum][index_bf], Gamma - 1);
							}
							if(NumberOfColours > 0)
							{
...
//        	  TRACEF("      zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz step, ii, jj, kk = %lld %lld *", ii, jj);
						...
						for(kk = 0; kk < GridDimension[2]; kk++)
						{
							...
							BaryonField[Vel1Num][index_bf] = field_line[kk + line_size * 2]
									/ field_line[kk + line_size * 0];
							BaryonField[Vel2Num][index_bf] = field_line[kk + line_size * 3]
									/ field_line[kk + line_size * 0];
							BaryonField[Vel3Num][index_bf] = field_line[kk + line_size * 1]
									/ field_line[kk + line_size * 0];
							BaryonField[B1Num][index_bf] = field_line[kk + line_size * 5];
							BaryonField[B2Num][index_bf] = field_line[kk + line_size * 6];
							BaryonField[B3Num][index_bf] = field_line[kk + line_size * 4];
							if(EquationOfState == 0)
							{
								BaryonField[TENum][index_bf] = field_line[kk + line_size * 7];
								pressure[index_bf] = field_line[kk + line_size * 8]
										* POW(field_line[kk + line_size * 0], (Gamma - 1));
							}
							if(NumberOfColours > 0)
							{
								for(nColour = 0; nColour < NumberOfColours; nColour++)
						...
						for(subgrid = 0; subgrid < NumberOfSubgrids; subgrid++)
						{
							...
							{
								offset = (ii - fistart[dim][subgrid])
										+ (jj - fjstart[dim][subgrid]) * nfi[dim][subgrid];
								kk = lindex[dim][subgrid];
								SubgridFluxes[subgrid]->LeftFluxes[DensNum][dim][offset] = dtdx
										* flux_line[kk + line_size * 0];
								SubgridFluxes[subgrid]->LeftFluxes[Vel1Num][dim][offset] = dtdx
										* flux_line[kk + line_size * 2];
								SubgridFluxes[subgrid]->LeftFluxes[Vel2Num][dim][offset] = dtdx
										* flux_line[kk + line_size * 3];
								SubgridFluxes[subgrid]->LeftFluxes[Vel3Num][dim][offset] = dtdx
										* flux_line[kk + line_size * 1];
								if(EquationOfState == 0)
								{
									SubgridFluxes[subgrid]->LeftFluxes[TENum][dim][offset] = dtdx
											* flux_line[kk + line_size * 7];
								} //te flux
								if(DualEnergyFormalism)
								{
								...
								if(NumberOfColours > 0)
								{
									...
									{
										SubgridFluxes[subgrid]->LeftFluxes[colnum[nColour]][dim][offset] = dtdx
												* flux_colour[kk + line_size * nColour];
									}
								}
