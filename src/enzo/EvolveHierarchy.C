...
int CheckForOutput(HierarchyEntry *TopGrid, TopGridData &MetaData, ExternalBoundary *Exterior,
#ifdef TRANSFER
	ImplicitProblemABC *ImplicitSolver,
#endif
	int Restart = FALSE);
int CheckForTimeAction(LevelHierarchyEntry *LevelArray[], TopGridData &MetaData);
int CheckForResubmit(TopGridData &MetaData, int &Stop);
int CosmologyComputeExpansionFactor(FLOAT time, FLOAT *a, FLOAT *dadt);
int OutputLevelInformation(FILE *fptr, TopGridData &MetaData, LevelHierarchyEntry *LevelArray[]);
int PrepareGravitatingMassField(HierarchyEntry *Grid, TopGridData *MetaData, LevelHierarchyEntry *LevelArray[],
	int level);
int ReduceFragmentation(HierarchyEntry &TopGrid, TopGridData &MetaData, ExternalBoundary *Exterior,
...
int ParticleSplitter(LevelHierarchyEntry *LevelArray[], int ThisLevel, TopGridData *MetaData);
int MagneticFieldResetter(LevelHierarchyEntry *LevelArray[], int ThisLevel, TopGridData *MetaData);
...
int EvolveHierarchy(HierarchyEntry &TopGrid, TopGridData &MetaData, ExternalBoundary *Exterior,
#ifdef TRANSFER
	ImplicitProblemABC *ImplicitSolver,
#endif
	LevelHierarchyEntry *LevelArray[], float Initialdt)
{
	...
	LCAPERF_BEGIN("EL");LCAPERF_START("EvolveHierarchy");
  ...
  CommunicationBarrier();
	...
	if(MetaData.FirstTimestepAfterRestart == TRUE && ParticleSplitterIterations > 0)
		ParticleSplitter(LevelArray, 0, &MetaData);

	/* Reset magnetic fields if requested. */

	if(MetaData.FirstTimestepAfterRestart == TRUE && ResetMagneticField == TRUE)
  ...
  Eint32 lcaperf_iter;
	...
	LCAPERF_STOP("EvolveHierarchy");LCAPERF_END("EH");
    ...
    fprintf(memtracePtr, "==== CYCLE %"ISYM" ====\n", MetaData.CycleNumber);
		...
		if((CheckpointRestart == FALSE) && (!FirstLoop))
			CommunicationLoadBalanceRootGrids(LevelArray, MetaData.TopGridRank, MetaData.CycleNumber);
...
//            dt = min(1.0001*(CosmologyOutputRedshiftTime[i]-MetaData.Time), dt);
			for(i = 0; i < MAX_TIME_ACTIONS; i++)
				if(TimeActionTime[i] > 0 && TimeActionType[i] > 0)
					LimitDt(&dt, 1.0001 * (TimeActionTime[i] - MetaData.Time), &dtLimitInfo, MAX_DT_TIME_ACTION_REACHED,
							i);
//      dt = min(1.0001*(TimeActionTime[i] - MetaData.Time), dt);
			if(MetaData.dtDataDump > 0.0)
			{
				...
				LimitDt(&dt, 1.0001 * (MetaData.TimeLastDataDump + MetaData.dtDataDump - MetaData.Time), &dtLimitInfo,
						MAX_DT_DATA_DUMP_REACHED, MAX_DT_INDEX_NOT_APPLICABLE);
...
//              MetaData.Time), dt);
			...
			LimitDt(&dt, MetaData.StopTime - MetaData.Time, &dtLimitInfo, MAX_DT_STOP_TIME_REACHED,
					MAX_DT_INDEX_NOT_APPLICABLE);
...
*/
						...
						dt, SiblingGridListStorage) == FAIL)
		{
			if(NumberOfProcessors == 1)
			{
				...
				Group_WriteAllData(MetaData.DataDumpName, MetaData.DataDumpNumber, &TopGrid, MetaData, Exterior
#ifdef TRANSFER
									,
									ImplicitSolver
#endif
		...
		if(MyProcessorNumber == ROOT_PROCESSOR)
		{
			...
			else if(MetaData.CycleNumber >= MetaData.StopCycle)
			{
				if(MyProcessorNumber == ROOT_PROCESSOR)
					printf("Stopping on top grid cycle limit.\n");
				Stop = TRUE;
			}
			else if(MetaData.CPUTime >= MetaData.StopCPUTime)
			{
				if(MyProcessorNumber == ROOT_PROCESSOR)
					printf("Stopping on CPU time limit.\n");
				Stop = TRUE;
			}
			else if((ReturnWallTime() - MetaData.StartCPUTime >= MetaData.dtRestartDump && MetaData.dtRestartDump > 0)
					|| (MetaData.CycleNumber - MetaData.CycleLastRestartDump >= MetaData.CycleSkipRestartDump
  ...
  lcaperf.attribute ("timestep",0, LCAPERF_NULL);
	...
	if(MyProcessorNumber == ROOT_PROCESSOR)
	{
		printf("Time     = %9"FSYM"   CycleNumber = %6"ISYM"    Wallclock   = %9"FSYM"\n", MetaData.Time,
				MetaData.CycleNumber, MetaData.CPUTime);
	...
	if((MetaData.dtDataDump != 0.0 || MetaData.CycleSkipDataDump != 0) && !MetaData.WroteData)
		//#ifdef USE_HDF5_GROUPS
		if(Group_WriteAllData(MetaData.DataDumpName, MetaData.DataDumpNumber, &TopGrid, MetaData, Exterior,
#ifdef TRANSFER
								ImplicitSolver,
#endif
								-666) == FAIL)
