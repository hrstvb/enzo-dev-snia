		 ...
		 int &FlaggedGrids)
{
  ...
  // Signal invocation before FlagBufferZones via NumberOfFlagedCells
  // which is should be  >=0 or INT_UNDEFINED.
//  CurrentGrid->SetFlaggingFieldStaticRegions(level, NumberOfFlaggedCells);
...
//	  NumberOfFlaggedCells = -1; // Signal invocation after FlagBufferZones
	  // Forget the original value,
	  // SetFlaggingFieldStatic does recount at the end.
