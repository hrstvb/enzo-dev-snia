...
int readAttribute( const hid_t loc_id, 
		   ...
		   const char* attrName, 
		   void* result,
		   const bool  callAssert)
{

  hid_t attr;
  int err = 0;

  err |= checkErr ( attr = H5Aopen_name(loc_id,attrName),attrName, callAssert );  
  err |= checkErr ( H5Aread(attr,type_id,result),attrName, callAssert ); 
  err |= checkErr ( H5Aclose(attr),attrName, callAssert );               
