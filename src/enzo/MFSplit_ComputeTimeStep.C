...
/  modified1:  
/
/  PURPOSE: Computes the module time step sizes.  We note that this 
/           value affects the global hydrodynamics time step: 
/                 dt = min(dt_hydro,dt_radiation).
/           This routine is called with scaled arguments.
/
/           Also, the argument 'flag' determines which entries we use 
/           to compute the time step:
/                   0 -> use radiation fields only
/                   1 -> use chemistry & energy corretion only
/                   2 -> use both
/
************************************************************************/
#ifdef TRANSFER
...
float MFSplit::ComputeTimeStep(EnzoVector *unew, int flag)
{
  ...
  if (Nx != LocDims[0]) 
    ENZO_FAIL("MFSplit ComputeTimeStep: x0 vector dims do not match\n");
  if (Ny != LocDims[1]) 
    ENZO_FAIL("MFSplit ComputeTimeStep: x1 vector dims do not match\n");
  if (Nz != LocDims[2]) 
    ENZO_FAIL("MFSplit ComputeTimeStep: x2 vector dims do not match\n");
  if (Nvar != (4+Nchem)) 
    ENZO_FAIL("MFSplit ComputeTimeStep: nspecies dims do not match\n");
  if ((Nx+ghXl+ghXr) != ArrDims[0]) 
    ENZO_FAIL("MFSplit ComputeTimeStep: x0 vector sizes do not match\n");
  if ((Ny+ghYl+ghYr) != ArrDims[1]) 
    ENZO_FAIL("MFSplit ComputeTimeStep: x1 vector sizes do not match\n");
  if ((Nz+ghZl+ghZr) != ArrDims[2]) 
    ENZO_FAIL("MFSplit ComputeTimeStep: x2 vector sizes do not match\n");


  // set internal dtfactor array based on user inputs and flag:
  float dtfactor[3];
  int i;
  for (i=0; i<3; i++)  dtfactor[i] = huge_number;
  if ((flag == 0) || (flag == 2))  dtfactor[0] = dtfac[0];
  if (flag > 0)  {
    dtfactor[1] = dtfac[1];
    dtfactor[2] = dtfac[2];
  }
  ...
  int j, k, l, idx;
  for (i=0; i<4+Nchem; i++)  test = min(dtfactor[i],test);
  if (test != huge_number) {
    ...
    if (debug) {
      printf("    MFSplit: dt_est(E1,E2,E3,e,ne) = (");
      for (l=0; l<5; l++) {
	if (dt_est_var[l] == huge_number/TimeUnits)
	  printf(" -----");
	else
	  printf(" %5.0e",dt_est_var[l]);
      }
      printf(")\n");
    }
  }
  ...
  // account for min/max time step size (according to user)
  dt_est = min(max(dt_est, mindt), maxdt);
