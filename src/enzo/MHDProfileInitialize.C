...
FLOAT gamma)
{
	...
	fprintf(file, "# rownum r                dr               rho              P                U                g            v_r\n");
...
//	}
	...
	if(debug && MyProcessorNumber == ROOT_PROCESSOR)
	{
		for(size_t i = 0; i < N; i++)
		{
			...
			P = PP[i];
			U = UU[i];
			TRACEF("Intergrated ressure: %lld: r=%e P=%e U=%e rho=%e gamma-1=%f", i, r, P, U, rho, gammaMinusOne);
		}
	}
