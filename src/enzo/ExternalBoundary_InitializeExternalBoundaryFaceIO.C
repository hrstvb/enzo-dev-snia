...
int WRITE_BT(boundary_type *bt_buffer, int field, int dim, int face, int slabsize, int BoundaryDimension[],
	int BoundaryRank, int Nfields);
int WRITE_BV(float *bv_buffer, int field, int dim, int face, int slabsize, int BoundaryDimension[], int BoundaryRank,
	int Nfields);
#endif
...
int ExternalBoundary::InitializeExternalBoundaryFace(int dim, boundary_type LeftBoundaryType,
	boundary_type RightBoundaryType, float LeftBoundaryValue[], float RightBoundaryValue[])
{
	int field, index;
		...
		for(field = 0; field < NumberOfBaryonFields; field++)
		{
			if(debug1)
				printf("InitializeBoundary: field %d, allocating %d bytes\n", field,
						2 * sizeof(boundary_type) * size / BoundaryDimension[dim]);

			BoundaryType[field][dim][0] = new boundary_type[size / BoundaryDimension[dim]];
			BoundaryType[field][dim][1] = new boundary_type[size / BoundaryDimension[dim]];
