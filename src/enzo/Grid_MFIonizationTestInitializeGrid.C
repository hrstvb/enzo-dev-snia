/***********************************************************************
/
/  GRID CLASS (INITIALIZE THE MULTI-FREQUENCY RAD-HYDRO IONIZATION TEST) 
/
...
int grid::MFIonizationTestInitializeGrid(int NumChemicals,
...
#ifdef TRANSFER
//   if (debug)
//     fprintf(stdout,"Entering grid::MFIonizationTestInitializeGrid routine\n");
  ...
  FieldType[FSNum = NumberOfBaryonFields++]    = RadiationFreq0;
  ...
  if (NewData == TRUE) {
    ...
    float E1Units = rUnits*E1Scale;
    float E2Units = rUnits*E2Scale;
    float E3Units = rUnits*E3Scale;
    ...
    for (i=0; i<size; i++) {
      ...
      BaryonField[FSNum][i]    = FSConstant/rUnits;
      BaryonField[E1Num][i]    = E1Constant/E1Units;
      BaryonField[E2Num][i]    = E2Constant/E2Units;
      BaryonField[E3Num][i]    = E3Constant/E3Units;
	...
	printf("   internal energy = %g\n",IEConstant);
      ...
      fprintf(stdout,"   fs-radiation = %g\n",FSConstant);
      fprintf(stdout,"     radiation1 = %g\n",E1Constant);
      fprintf(stdout,"     radiation2 = %g\n",E2Constant);
      fprintf(stdout,"     radiation3 = %g\n",E3Constant);
  ...
  ENZO_FAIL("MFIonizationTestInitializeGrid: TRANSFER must be enabled for this test!");
