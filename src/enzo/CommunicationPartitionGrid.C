...
int CommunicationPartitionGrid(HierarchyEntry *Grid, int gridnum)
{
	...
	float Edge = POW(float(Dims[0]*Dims[1]*Dims[2])/float(NumberOfProcessors), 1/float(Rank));

	/* If using MPI, use their routine to calculate layout. */
#ifdef USE_MPI
	/* If defined, use user defined Processor Topology. */
	if(UserDefinedRootGridLayout[0] != INT_UNDEFINED && UserDefinedRootGridLayout[1] != INT_UNDEFINED
	...
	for(dim = 0; dim < MAX_DIMENSION; dim++)
	{
		...
		for(i = 0; i < Layout[dim]; i++)
		{
			...
			/* Compute the integer number of cells along this dimension
			 (if dim == 0 then make sure it is even as well since the FFT requires this). */
			if(i < Layout[dim] - 1)
	...
	if(MyProcessorNumber == ROOT_PROCESSOR)
	{
		...
		if(Grid->ParentGrid != NULL)
		{
			...
			else
			{
				...
				Grid->ParentGrid->GridData->ReturnGridInfo(&Rank, ParentDims, ParentLeftEdge, ParentRightEdge);
				for(i = 0; i < MAX_STATIC_REGIONS; i++)
				{
			...
			for(dim = 0; dim < MAX_DIMENSION; dim++)
			{
				...
				while(ThisSlab < Layout[dim])
				{
					...
					if(CoarseSlab < NumberOfCoarseSlabs + FirstCoarseSlab && CoarseSlab < Layout[dim])
	...
	if(MyProcessorNumber == ROOT_PROCESSOR)
	{
		...
		for(dim = 0; dim < MAX_DIMENSION; dim++)
			printf("GridDims[%" ISYM "]: %" ISYM " %" ISYM " %" ISYM "\n", dim, GridDims[dim][0], GridDims[dim][1],
					GridDims[dim][2]);
		for(dim = 0; dim < MAX_DIMENSION; dim++)
			printf("StartIndex[%" ISYM "]: %" ISYM " %" ISYM " %" ISYM "\n", dim, StartIndex[dim][0],
					StartIndex[dim][1], StartIndex[dim][2]);
	}
	...
	for(k = 0; k < Layout[2]; k++)
	{
		for(j = 0; j < Layout[1]; j++)
		{
			for(i = 0; i < Layout[0]; i++)
			{
				...
				if(!ParallelRootGridIO)
					if(OldGrid->ZeroSolutionUnderSubgrid(NewGrid, ZERO_UNDER_SUBGRID_FIELD,
					float(gridcounter + 1)) == FAIL)
					{
						ENZO_FAIL("Error in grid->ZeroSolutionUnderSubgrid.");
					}

				// This will assign unique IDs only if there is a single grid on this level. [BH]
				NewGrid->SetGridID(gridcounter); //TODO [BH]
	...
	if(LoadBalancing == 4)
		LoadBalanceHilbertCurve(SubGrids, gridcounter, PartitionProcessorNumbers);
