...
int grid::MHDSourceTerms(float **dU)
{
	...
	if(DualEnergyFormalism)
	{
		...
		FLOAT coef = 0.5;
		FLOAT dtdx = coef * dtFixed / CellWidth[0][0] / a, dtdy =
				(GridRank > 1) ? coef * dtFixed / CellWidth[1][0] / a : 0.0, dtdz =
				(GridRank > 2) ? coef * dtFixed / CellWidth[2][0] / a : 0.0;
		float min_coeff = 0.0;
		if(UseMinimumPressureSupport)
		{
			min_coeff = MinimumPressureSupportParameter * 0.32 * pow(CellWidth[0][0], 2) / (Gamma * (Gamma - 1.0));
		}
	...
	{
		float rho, etot, eint, vx, vy, vz, v2, e, h, cs, p, dpdrho, dpde, coty, Bx, By, Bz, B2;
		FLOAT x, dtxinv;
		int n = 0, igrid;
		for(int k = GridStartIndex[2]; k <= GridEndIndex[2]; k++)
		{
		...
		for(int k = GridStartIndex[2]; k <= GridEndIndex[2]; k++)
		{
			for(int j = GridStartIndex[1]; j <= GridEndIndex[1]; j++)
			{
				for(int i = GridStartIndex[0]; i <= GridEndIndex[0]; i++, n++)
				{
					...
					dU[iEtot][n] -= dtFixed * rho
							* (GasDragCoefficient * vx * vx + GasDragCoefficient * vy * vy
	...
	if((SelfGravity) || ExternalGravity || UniformGravity || PointSourceGravity || UseSphericalGravity)
	{
		int igrid;
		float rho, gx, gy, gz;
		float vx, vy, vz, vx_old, vy_old, vz_old;
	...
	{ // add some B related cosmological expansion terms here
	...
	if((ProblemType == 35 || ProblemType == 36 || ProblemType == 37) && ShearingBoxProblemType != 0)
	{
		...
		float lengthz;
		if(GridRank == 3)
			lengthz = DomainRightEdge[2] - DomainLeftEdge[2];
		else
			lengthz = 0.0;
		...
		for(int k = GridStartIndex[2]; k <= GridEndIndex[2]; k++)
		{
			for(int j = GridStartIndex[1]; j <= GridEndIndex[1]; j++)
			{
				for(int i = GridStartIndex[0]; i <= GridEndIndex[0]; i++, n++)
				{
					...
					dU[indexNumbers[ShearingBoundaryDirection]][n] += dtFixed * 2.0 * rho * VelocityGradient
							* AngularVelocity * AngularVelocity * xPos[ShearingBoundaryDirection];

					dU[iEtot][n] += dtFixed * 2.0 * rho * VelocityGradient * AngularVelocity * AngularVelocity
							* xPos[ShearingBoundaryDirection] * vels[ShearingBoundaryDirection];
