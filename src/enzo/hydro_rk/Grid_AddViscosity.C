...
int grid::AddViscosity()
{
  ...
  if(UseViscosity == 3)
  {
  	...
  	float *e = BaryonField[TENum];
  	size_t index;
  	double kinE, maxMag = 0;
  	int maxI, maxJ, maxK;
  	for(int k = 0; k < GridDimension[2]; k++)
  	{
  		for(int j = 0; j < GridDimension[1]; j++)
  		{
  			...
  			{
  				index = ELT(i, j, k);
  				kinE = square(u[index]) + square(v[index]) + square(w[index]);
  				if(maxMag < kinE)
  				{
  					maxMag = kinE;
  					maxI = i;
  				...
  				kinE /= 2;
  				if(0 >= (e[index] -= kinE))
  					TRACEGF(" Total energy, %e < 0, after subtracting the kinetic, %e at %e %e %e ( %lld %lld %lld )",
  							e[index], kinE, CELLCENTER(0, i), CELLCENTER(1,j), CELLCENTER(2, k), i, j, k);
  				// u[index] = v[index] = w[index] = 0;
  			}
  		}
  	}
  	arr_set(u, GetGridSize(), 0);
  	arr_set(v, GetGridSize(), 0);
  	arr_set(w, GetGridSize(), 0);
  	maxMag = sqrt(maxMag);
  	TRACEGF(" MAX velocity magnitude: %e  at  %e %e %e ( %lld %lld %lld )", maxMag, CELLCENTER(0, maxI),
  			CELLCENTER(1,maxJ), CELLCENTER(2, maxK), maxI, maxJ, maxK);
