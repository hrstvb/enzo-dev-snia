...
float dtdx, char direc, int jj, int kk, int fallback, int debug);
...
//  int icons = (ConservativeReconstruction) ? 1 : 0;  // not implemented properly yet, TA
	...
	float *priml[NEQ_MHD - idual + (bool) UseBurning],  //[BH]
			*primr[NEQ_MHD - idual + (bool) UseBurning],  //[BH]
			*species[NSpecies], *colors[NColor];
	...
	for(int field = 0; field < NEQ_MHD + NSpecies + NColor - idual+(bool)UseBurning; field++)
	{
		Prim1[field] = new float[GridDimension[0]];
	}

	int extra = (ReconstructionMethod == PPM);
	for(int field = 0; field < NEQ_MHD - idual + (bool) UseBurning; field++)
	{
	...
	for(k = 0; k < Zactivesize; k++)
	{
		for(j = 0; j < Yactivesize; j++)
		{
			...
			int debug = 0 && k == Zactivesize / 2 && j == Yactivesize / 2;

			for(i = 0; i < GridDimension[0]; i++)
			{
				// We only use density, internal energy, vi to do the reconstruction
				igrid = i + (j + GridStartIndex[1] + (k + GridStartIndex[2]) * GridDimension[1]) * GridDimension[0];
				rho = Prim[iden][igrid];
				vx = Prim[ivx][igrid];
				vy = Prim[ivy][igrid];
				vz = Prim[ivz][igrid];
...
//      }
			...
			if(MHDLine(Prim1, priml, primr, species, colors, FluxLine, Xactivesize, dtdx, 'x', j, k, fallback,
						debug) == FAIL)
			{
