...
int hll_mhd(float **FluxLine, float **priml, float **primr, float **prim, int ActiveSize, int debug)
{
  ...
  for (int n = 0; n < ActiveSize+1; n++) {
    ...
    if(UseBurning) //[BH]
    {
    	...
    	float fl = ul * vl;
    	float fr = ur * vr;
    	float flux = (ap * fl + am * fr - ap * am * (ur - ul)) / (ap + am);
    	...
    	//float flux = rhoFlux * (rhoFlux >= 0) ? (ul / rhoL) : (ur / rhoR);
...
{
	return hll_mhd(FluxLine, priml, primr, prim, ActiveSize, 0);
}
