...
int grid::SolveHydroEquations(int CycleNumber, int NumberOfSubgrids, fluxes *SubgridFluxes[], int level)
{
	...
	LCAPERF_START("grid_SolveHydroEquations"); TIMER_START("SolveHydroEquations");
	...
	if(NumberOfBaryonFields > 0)
	{
		...
		if(RadiativeTransferFLD == 2)
		{
			...
			{  // grey radiation problem
				...
				//   HIDensity
				if((ColourNum = FindField(HIDensity, FieldType, NumberOfBaryonFields)) >= 0)
					colnum[++NumberOfColours] = ColourNum;
				//   HIIDensity
				if((ColourNum = FindField(HIIDensity, FieldType, NumberOfBaryonFields)) >= 0)
					colnum[++NumberOfColours] = ColourNum;
				//   HeIDensity
				if((ColourNum = FindField(HeIDensity, FieldType, NumberOfBaryonFields)) >= 0)
					colnum[++NumberOfColours] = ColourNum;
				//   HeIIDensity
				if((ColourNum = FindField(HeIIDensity, FieldType, NumberOfBaryonFields)) >= 0)
					colnum[++NumberOfColours] = ColourNum;
				//   HeIIIDensity
				if((ColourNum = FindField(HeIIIDensity, FieldType, NumberOfBaryonFields)) >= 0)
					colnum[++NumberOfColours] = ColourNum;
				//   HMDensity
				if((ColourNum = FindField(HMDensity, FieldType, NumberOfBaryonFields)) >= 0)
					colnum[++NumberOfColours] = ColourNum;
				//   H2IDensity
				if((ColourNum = FindField(H2IDensity, FieldType, NumberOfBaryonFields)) >= 0)
					colnum[++NumberOfColours] = ColourNum;
				//   H2IIDensity
				if((ColourNum = FindField(H2IIDensity, FieldType, NumberOfBaryonFields)) >= 0)
					colnum[++NumberOfColours] = ColourNum;
				//   DIDensity
				if((ColourNum = FindField(DIDensity, FieldType, NumberOfBaryonFields)) >= 0)
					colnum[++NumberOfColours] = ColourNum;
				//   DIIDensity
				if((ColourNum = FindField(DIIDensity, FieldType, NumberOfBaryonFields)) >= 0)
					colnum[++NumberOfColours] = ColourNum;
				//   HDIDensity
				if((ColourNum = FindField(HDIDensity, FieldType, NumberOfBaryonFields)) >= 0)
					colnum[++NumberOfColours] = ColourNum;
			}
		}
		...
		if(this->IdentifyColourFields(SNColourNum, MetalNum, MetalIaNum, MetalIINum, MBHColourNum, Galaxy1ColourNum,
		...
		if(MetalIaNum != -1)
			colnum[NumberOfColours++] = MetalIaNum;
		if(MetalIINum != -1)
			colnum[NumberOfColours++] = MetalIINum;
		if(SNColourNum != -1)
			colnum[NumberOfColours++] = SNColourNum;
		if(MBHColourNum != -1)
			colnum[NumberOfColours++] = MBHColourNum;
		if(Galaxy1ColourNum != -1)
			colnum[NumberOfColours++] = Galaxy1ColourNum;
		if(Galaxy2ColourNum != -1)
			colnum[NumberOfColours++] = Galaxy2ColourNum;
		...
		if(TestProblemData.GloverChemistryModel)
		{
			...
			int DeNum, HINum, HIINum, HeINum, HeIINum, HeIIINum, HMNum, H2INum, H2IINum, DINum, DIINum, HDINum;

			int CINum, CIINum, OINum, OIINum, SiINum, SiIINum, SiIIINum, CHINum, CH2INum, CH3IINum, C2INum, COINum,
					HCOIINum, OHINum, H2OINum, O2INum;

			int GCM = TestProblemData.GloverChemistryModel;  // purely for convenience

			if(IdentifyGloverSpeciesFields(HIINum, HINum, H2INum, DINum, DIINum, HDINum, HeINum, HeIINum, HeIIINum,
											CINum, CIINum, OINum, OIINum, SiINum, SiIINum, SiIIINum, CHINum, CH2INum,
											CH3IINum, C2INum, COINum, HCOIINum, OHINum, H2OINum, O2INum) == FAIL)
			{
		...
		if(CRModel)
		{
			...
			if(this->IdentifyPhysicalQuantities(DensNum, GENum, Vel1Num, Vel2Num, Vel3Num, TENum, CRNum) == FAIL)
...
//printf("New numcols %d (+%d) \n", NumberOfColours, NumberOfColours-oldNumcol);	//[BH]
		...
		for(i = 0; i < NumberOfSubgrids; i++)
		{
			for(dim = 0; dim < GridRank; dim++)
			{
				...
				for(field = NumberOfBaryonFields; field < MAX_NUMBER_OF_BARYON_FIELDS; field++)
				{
		...
		if(UseMHDCT)
		{
			...
			for(field = 0; field < 3; field++)
			{
				...
				for(i = 0; i < 2 * MagneticSize[field]; i++)
		...
		int CurlStart[3] = { 0, 0, 0 }, CurlEnd[3] =
				{ GridDimension[0] - 1, GridDimension[1] - 1, GridDimension[2] - 1 };
		if(UseMHDCT)
		{
		...
		if(HydroMethod == PPM_DirectEuler)
			this->SolvePPM_DE(CycleNumber, NumberOfSubgrids, SubgridFluxes, CellWidthTemp, GridGlobalStart, GravityOn,
								NumberOfColours, colnum, MinimumSupportEnergyCoefficient);
      ...
      FORTRAN_NAME(ppm_lr)(
		...
		if(HydroMethod == Zeus_Hydro)
			if(this->ZeusSolver(GammaField, UseGammaField, CycleNumber, CellWidthTemp[0], CellWidthTemp[1],
								CellWidthTemp[2], GravityOn, NumberOfSubgrids, GridGlobalStart, SubgridFluxes,
								NumberOfColours, colnum, LowestLevel, MinimumSupportEnergyCoefficient) == FAIL)
		...
		if(OutputOnDensity == 1 || StopFirstTimeAtDensity > 0. || StopFirstTimeAtMetalEnrichedDensity > 0.)
		{
			...
			for(i = 0; i < size; i++)
			{
				...
				{
					CurrentMaximumMetalEnrichedDensity = max(BaryonField[DensNum][i],
	...
	TIMER_STOP("SolveHydroEquations");LCAPERF_STOP("grid_SolveHydroEquations");
	return SUCCESS;
