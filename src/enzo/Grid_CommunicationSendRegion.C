...
//    return SUCCESS;
	...
	int NumberOfFields = ((SendField == ALL_FIELDS) ? NumberOfBaryonFields : 1) * ((NewOrOld == NEW_AND_OLD) ? 2 : 1);
	...
	if(UseMHDCT)
	{
		...
		TransferSize += ((SendField == ALL_FIELDS) ? 3 * RegionSize : 0) * ((NewOrOld == NEW_AND_OLD) ? 2 : 1);
	...
	if(MyProcessorNumber == ProcessorNumber)
	{
		...
		if(NewOrOld == NEW_AND_OLD || NewOrOld == NEW_ONLY)
			for(field = 0; field < max(NumberOfBaryonFields, SendField + 1); field++)
				...
				{
					FORTRAN_NAME(copy3d)(BaryonField[field], &buffer[index], GridDimension, GridDimension + 1,
											GridDimension + 2, RegionDim, RegionDim + 1, RegionDim + 2, Zero, Zero + 1,
											Zero + 2, RegionStart, RegionStart + 1, RegionStart + 2);
					index += RegionSize;
				}
		...
		if(NewOrOld == NEW_AND_OLD || NewOrOld == OLD_ONLY)
			for(field = 0; field < max(NumberOfBaryonFields, SendField + 1); field++)
				if(field == SendField || SendField == ALL_FIELDS)
				{
					FORTRAN_NAME(copy3d)(OldBaryonField[field], &buffer[index], GridDimension, GridDimension + 1,
											GridDimension + 2, RegionDim, RegionDim + 1, RegionDim + 2, Zero, Zero + 1,
											Zero + 2, RegionStart, RegionStart + 1, RegionStart + 2);
					index += RegionSize;
				}
		...
		if(UseMHDCT && SendField == ALL_FIELDS)
		{
			...
			if(NewOrOld == NEW_AND_OLD || NewOrOld == NEW_ONLY)
			{
				...
				for(field = 0; field < 3; field++)
				{
					FORTRAN_NAME(copy3d)(MagneticField[field], &buffer[index], &MagneticDims[field][0],
											&MagneticDims[field][1], &MagneticDims[field][2], &MHDRegionDim[field][0],
											&MHDRegionDim[field][1], &MHDRegionDim[field][2], Zero, Zero + 1, Zero + 2,
			...
			if(NewOrOld == NEW_AND_OLD || NewOrOld == OLD_ONLY)
			{
				for(field = 0; field < 3; field++)
				{
					FORTRAN_NAME(copy3d)(OldMagneticField[field], &buffer[index], &MagneticDims[field][0],
											&MagneticDims[field][1], &MagneticDims[field][2], &MHDRegionDim[field][0],
											&MHDRegionDim[field][1], &MHDRegionDim[field][2], Zero, Zero + 1, Zero + 2,
											RegionStart, RegionStart + 1, RegionStart + 2);
					index += MHDRegionSize[field];
		...
		if(SendField == GRAVITATING_MASS_FIELD_PARTICLES)
			FORTRAN_NAME(copy3d)(GravitatingMassFieldParticles, buffer, GravitatingMassFieldParticlesDimension,
									GravitatingMassFieldParticlesDimension + 1,
									GravitatingMassFieldParticlesDimension + 2, RegionDim, RegionDim + 1, RegionDim + 2,
									Zero, Zero + 1, Zero + 2, RegionStart, RegionStart + 1, RegionStart + 2);

		if(SendField == GRAVITATING_MASS_FIELD)
			FORTRAN_NAME(copy3d)(GravitatingMassField, buffer, GravitatingMassFieldDimension,
									GravitatingMassFieldDimension + 1, GravitatingMassFieldDimension + 2, RegionDim,
									RegionDim + 1, RegionDim + 2, Zero, Zero + 1, Zero + 2, RegionStart,
									RegionStart + 1, RegionStart + 2);

		if(SendField == POTENTIAL_FIELD)
			FORTRAN_NAME(copy3d)(PotentialField, buffer, GravitatingMassFieldDimension,
									GravitatingMassFieldDimension + 1, GravitatingMassFieldDimension + 2, RegionDim,
									RegionDim + 1, RegionDim + 2, Zero, Zero + 1, Zero + 2, RegionStart,
									RegionStart + 1, RegionStart + 2);
		...
		if(SendField == ACCELERATION_FIELDS)
			...
			{
				FORTRAN_NAME(copy3d)(AccelerationField[dim], &buffer[index], GridDimension, GridDimension + 1,
										GridDimension + 2, RegionDim, RegionDim + 1, RegionDim + 2, Zero, Zero + 1,
										Zero + 2, RegionStart, RegionStart + 1, RegionStart + 2);
				index += RegionSize;
			}
...
#endif
			TRACEF("sending %lld (%"ISYM " %"ISYM " %" ISYM ") %lldx%lldx%lld -> %lld", TransferSize, RegionStart[0],
					RegionStart[1], RegionStart[2], RegionDim[0], RegionDim[1], RegionDim[2], ToProcessor);
			CommunicationBufferedSend(buffer, TransferSize, DataType, ToProcessor, MPI_SENDREGION_TAG, MPI_COMM_WORLD,
										BUFFER_IN_PLACE);
		}
...
//	       CommunicationReceiveIndex);
				TRACEF("post_receive %lld <-- %lld", ToProcessor, ProcessorNumber);
			...
			{
				TRACEF("send_receive %lld <-- %lld", ToProcessor, ProcessorNumber);
				MPI_Recv(buffer, TransferSize, DataType, ProcessorNumber, MPI_SENDREGION_TAG, MPI_COMM_WORLD, &Status);
			}
	...
	if(MyProcessorNumber == ToProcessor
			&& (CommunicationDirection == COMMUNICATION_SEND_RECEIVE || CommunicationDirection == COMMUNICATION_RECEIVE))
	{
...
//	      MyProcessorNumber, ProcessorNumber);
		...
		if(NewOrOld == NEW_AND_OLD || NewOrOld == NEW_ONLY)
			for(field = 0; field < max(NumberOfBaryonFields, SendField + 1); field++)
				if(field == SendField || SendField == ALL_FIELDS)
				{
					...
					FORTRAN_NAME(copy3d)(&buffer[index], ToGrid->BaryonField[field], RegionDim, RegionDim + 1,
											RegionDim + 2, RegionDim, RegionDim + 1, RegionDim + 2, Zero, Zero + 1,
											Zero + 2, Zero, Zero + 1, Zero + 2);
					index += RegionSize;
				}
		...
		if(NewOrOld == NEW_AND_OLD || NewOrOld == OLD_ONLY)
			for(field = 0; field < max(NumberOfBaryonFields, SendField + 1); field++)
				if(field == SendField || SendField == ALL_FIELDS)
				{
					...
					FORTRAN_NAME(copy3d)(&buffer[index], ToGrid->OldBaryonField[field], RegionDim, RegionDim + 1,
											RegionDim + 2, RegionDim, RegionDim + 1, RegionDim + 2, Zero, Zero + 1,
											Zero + 2, Zero, Zero + 1, Zero + 2);
					index += RegionSize;
				}
		...
		if(UseMHDCT && SendField == ALL_FIELDS)
		{
			...
			if(NewOrOld == NEW_AND_OLD || NewOrOld == NEW_ONLY)
			{
				for(field = 0; field < 3; field++)
				{
					...
					FORTRAN_NAME(copy3d)(&buffer[index], MagneticField[field], &MHDRegionDim[field][0],
											&MHDRegionDim[field][1], &MHDRegionDim[field][2], &MHDRegionDim[field][0],
											&MHDRegionDim[field][1], &MHDRegionDim[field][2], Zero, Zero + 1, Zero + 2,
											Zero, Zero + 1, Zero + 2);
					index += MHDRegionSize[field];
				}
			...
			if(NewOrOld == NEW_AND_OLD || NewOrOld == OLD_ONLY)
				for(field = 0; field < 3; field++)
				{
					...
					FORTRAN_NAME(copy3d)(&buffer[index], OldMagneticField[field], &MHDRegionDim[field][0],
											&MHDRegionDim[field][1], &MHDRegionDim[field][2], &MHDRegionDim[field][0],
											&MHDRegionDim[field][1], &MHDRegionDim[field][2], Zero, Zero + 1, Zero + 2,
											Zero, Zero + 1, Zero + 2);
					index += MHDRegionSize[field];
		...
		if(SendField == GRAVITATING_MASS_FIELD_PARTICLES)
		{
			...
			FORTRAN_NAME(copy3d)(buffer, ToGrid->GravitatingMassFieldParticles, RegionDim, RegionDim + 1, RegionDim + 2,
									RegionDim, RegionDim + 1, RegionDim + 2, Zero, Zero + 1, Zero + 2, Zero, Zero + 1,
									Zero + 2);
		}
		...
		if(SendField == GRAVITATING_MASS_FIELD)
		{
			...
			FORTRAN_NAME(copy3d)(buffer, ToGrid->GravitatingMassField, RegionDim, RegionDim + 1, RegionDim + 2,
									RegionDim, RegionDim + 1, RegionDim + 2, Zero, Zero + 1, Zero + 2, Zero, Zero + 1,
									Zero + 2);
		}
		...
		if(SendField == POTENTIAL_FIELD)
		{
			...
			FORTRAN_NAME(copy3d)(buffer, ToGrid->PotentialField, RegionDim, RegionDim + 1, RegionDim + 2, RegionDim,
									RegionDim + 1, RegionDim + 2, Zero, Zero + 1, Zero + 2, Zero, Zero + 1, Zero + 2);
		}
		...
		if(SendField == ACCELERATION_FIELDS)
			for(dim = 0; dim < GridRank; dim++)
			{
				...
				FORTRAN_NAME(copy3d)(&buffer[index], ToGrid->AccelerationField[dim], RegionDim, RegionDim + 1,
										RegionDim + 2, RegionDim, RegionDim + 1, RegionDim + 2, Zero, Zero + 1,
										Zero + 2, Zero, Zero + 1, Zero + 2);
				index += RegionSize;
			}
