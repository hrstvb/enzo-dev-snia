...
int grid::SetExternalBoundaryValues(ExternalBoundary *Exterior)
{
	...
	if(ProblemType == 4)
		if(Exterior->SetDoubleMachBoundary(Time, CellLeftEdge[0], CellWidth[0]) == FAIL)
		{
	...
	if(ProblemType == 201 && (EOSSoundSpeed > 0))
		if(Exterior->SetWengenCollidingFlowBoundary(Time, CellLeftEdge[0], CellWidth[0]) == FAIL)
		{
	...
	for(dim = 0; dim < MAX_DIMENSION; dim++)
		if(dim < GridRank)
			GridOffset[dim] = nint((GridLeftEdge[dim] - DomainLeftEdge[dim]) / CellWidth[dim][0]);
		else
			GridOffset[dim] = 0;
	...
	for(field = 0; field < NumberOfBaryonFields; field++)
	{
		...
		if(Exterior->SetExternalBoundary(GridRank, GridDimension, GridOffset, GridStartIndex, GridEndIndex,
											BaryonField[field], FieldType[field]) == FAIL)
		{
	...
	if(UseMHDCT && NumberOfBaryonFields > 0)
	{
		...
		if(Exterior->SetMagneticBoundary(GridRank, GridDimension, GridOffset, GridStartIndex, GridEndIndex,
	...
	if(NumberOfParticles > 0)
		...
		if(Exterior->SetExternalBoundaryParticles(GridRank, NumberOfParticles, ParticlePosition,
