...
int MFProb::SetupBoundary(int Dim, int Face, int BdryConst, float *BdryData) 
{
//   if (debug)
//     printf("Entering MFProb::SetupBoundary routine\n");
  ...
  // Error check
  if ((Dim < 0) || (Dim >= rank)) 
    ENZO_VFAIL("MFProb SetupBoundary: Dim %"ISYM" out of bounds.\n", Dim)
  if ((Face != 0) && (Face != 1)) 
    ENZO_VFAIL("MFProb SetupBoundary: Face %"ISYM" != {0,1}.\n", Face)
