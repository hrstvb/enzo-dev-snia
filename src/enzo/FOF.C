...
int link_across(FOFData &AllVars)
{
  ...
  int    leftTask, rightTask, nlinktot;
  id_data *iddat;
...
//    printf("linking..."); fflush(stdout);
  ...
  for (i = AllVars.Grid-2; i >= 1; i--)
	...
	if ((p = AllVars.GridFirst[i][j][k]) == TRUE) {
	  do {
	  ...
	  } while ((p = AllVars.GridNext[p]) == TRUE);
	} // ENDIF
      } // ENDFOR k
...
void check_cell(FOFData &AllVars, int p, int i, int j, int k)
{
  ...
  if ((s = AllVars.GridFirst[i][j][k]) == TRUE) {
    flag = AllVars.Head[s];
