...
int grid::CopyZonesFromGrid(grid *OtherGrid, FLOAT EdgeOffset[MAX_DIMENSION])
{
	...
	if(NumberOfBaryonFields == 0)
		return SUCCESS;
	TRACEF(" %lld (%p on %lld) --%lld-> %lld (%p on %lld)", ID, this, ProcessorNumber, OtherGrid->ID,
			CommunicationDirection, OtherGrid, OtherGrid->ProcessorNumber);
	...
	bool shiftPos, shiftNeg;
	float delta;
	FLOAT L;

	bool isShearing = ShearingBoundaryDirection != -1;
	if(ShearingBoundaryDirection != -1)
	{
		...
		shiftPos = fabs(EdgeOffset[ShearingBoundaryDirection] - L) <= CellWidth[ShearingBoundaryDirection][0] * 0.1;
		shiftNeg = fabs(EdgeOffset[ShearingBoundaryDirection] + L) <= CellWidth[ShearingBoundaryDirection][0] * 0.1;
		isShearing &= shiftNeg || shiftPos;
	}

	/* Compute the left and right edges of this grid (including ghost zones). */

	FLOAT GridLeft[MAX_DIMENSION];
	FLOAT GridRight[MAX_DIMENSION];

	for(dim = 0; dim < GridRank; dim++)
	{
		GridLeft[dim] = CellLeftEdge[dim][0] + EdgeOffset[dim];
		GridRight[dim] = CellLeftEdge[dim][GridDimension[dim] - 1] + CellWidth[dim][GridDimension[dim] - 1]
				+ EdgeOffset[dim];
	}
	...
	for(dim = 0; dim < GridRank; dim++)
		if(GridLeft[dim] >= OtherGrid->GridRightEdge[dim] || GridRight[dim] <= OtherGrid->GridLeftEdge[dim])
			return SUCCESS;
	...
	for(dim = 0; dim < GridRank; dim++)
	{
		if(GridDimension[dim] > 1)
		{
			...
			if(ShearingVelocityDirection == dim && isShearing)
			{
				...
				if(Start[dim] >= GridDimension[dim] || End[dim] >= GridDimension[dim])
					return SUCCESS;
			...
			StartOther[dim] = nint((Left[dim] - OtherGrid->CellLeftEdge[dim][0]) / CellWidth[dim][0]);

			if(isShearing && ShearingVelocityDirection == dim)
			{
				StartOther[dim] = (int) floor((Left[dim] - OtherGrid->CellLeftEdge[dim][0]) / CellWidth[dim][0]);
			}
...
//and that needs to be communicated from other grids possibly
...
#ifdef USE_MPI
	if(CommunicationDirection == COMMUNICATION_POST_RECEIVE && MyProcessorNumber == ProcessorNumber)
	{
		...
		for(dim = 0; dim < GridRank; dim++)
			CommunicationReceiveArgument[dim][CommunicationReceiveIndex] = EdgeOffset[dim];
	}
#endif /* USE_MPI */
	...
	if(traceMPI)
		fprintf(tracePtr, "CopyZones SendRegion from %"ISYM" to %"ISYM"\n", ProcessorNumber,
				OtherGrid->ProcessorNumber);

	if(ProcessorNumber != OtherGrid->ProcessorNumber)
	{
		OtherGrid->CommunicationSendRegion(OtherGrid, ProcessorNumber, ALL_FIELDS, NEW_ONLY, StartOther,
											ShearingCommunicationDims);

		if(CommunicationDirection == COMMUNICATION_POST_RECEIVE || CommunicationDirection == COMMUNICATION_SEND)
			return SUCCESS;
...
// If DualEnergyFormalism is turned off, subtract v and b from Etotal to get Eint
	...
	if(isShearing)
	{
		...
		if(DualEnergyFormalism)
			ieint = FindField(InternalEnergy, FieldType, NumberOfBaryonFields);
...
//   PrintToScreenBoundaries(BaryonField[ietot], "Etot before a copy");
	...
	if(!isShearing)
		for(field = 0; field < NumberOfBaryonFields; field++)
			FORTRAN_NAME(copy3drel)(OtherGrid->BaryonField[field], BaryonField[field], Dim, Dim + 1, Dim + 2, OtherDim,
									OtherDim + 1, OtherDim + 2, GridDimension, GridDimension + 1, GridDimension + 2,
									StartOther, StartOther + 1, StartOther + 2, Start, Start + 1, Start + 2);
					...
					for(i = 0; i < Dim[0]; i++, thisindex++, otherindex++)
					{
						...
						if(DualEnergyFormalism == 0 && FieldType[field] == TotalEnergy)
						{
							for(loop = 0; loop < 2; loop++)
							{
								...
								if(GridRank == 3)
									vz = OtherGrid->BaryonField[ivz][iLoop];
								else
									vz = 0.0;
								...
								if(UseMHD)
								{
									...
									if(GridRank == 3)
										bz = OtherGrid->BaryonField[iBz][iLoop];
									else
										bz = 0.0;
								...
								if(loop == 0)
									val1 = val1 - 0.5 * v2 - 0.5 * b2 / rho;
								else if(loop == 1)
									val2 = val2 - 0.5 * v2 - 0.5 * b2 / rho;
