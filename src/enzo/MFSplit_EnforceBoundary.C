...
/  modified1:  
/
/  PURPOSE: Enforces boundary conditions on a MFSplit vector.  
/
...
int MFSplit::EnforceBoundary(EnzoVector *u)
{

  float stime = MPI_Wtime();
  
//   if (debug)
//     printf("Entering MFSplit::EnforceBoundary routine\n");
  ...
  if (OnBdry[0][0] && (BdryType[0][0]==1)) 
	...
	for (i=0; i<ugh[0][0]; i++) {
	  ...
	  udata[idx] = EBdryVals[0][0][idxbc]/E1Units;
	}
  //   Neumann
  if (OnBdry[0][0] && (BdryType[0][0]==2)) {
	...
	idxbc = k*LocDims[1] + j;
	udata[idx] = udata[idx2] + dxa*EBdryVals[0][0][idxbc]/E1Units;
      }
  }
  ...
  if (OnBdry[0][1] && (BdryType[0][1]==1))
	...
	for (i=ArrDims[0]-ugh[0][1]; i<ArrDims[0]; i++) {
	  idxbc = k*LocDims[1] + j;
	  idx = ((k+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i;
	  udata[idx] = EBdryVals[0][1][idxbc]/E1Units;
	}
  //   Neumann
  if (OnBdry[0][1] && (BdryType[0][1]==2)) {
	...
	idx2 = ((k+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i2+ugh[0][0];
	idxbc = k*LocDims[1] + j;
	udata[idx] = udata[idx2] + dxa*EBdryVals[0][1][idxbc]/E1Units;
      }
  }
  ...
  if (rank > 1) {
	...
	for (j=0; j<ugh[1][0]; j++)
	  for (i=0; i<LocDims[0]; i++) {
	    ...
	    idxbc = i*LocDims[2] + k;
	    udata[idx] = EBdryVals[1][0][idxbc]/E1Units;
	  }
    //   Neumann
	...
	for (i=0; i<LocDims[0]; i++) {
	  ...
	  idxbc = i*LocDims[2] + k;
	  udata[idx] = udata[idx2] + dya*EBdryVals[1][0][idxbc]/E1Units;
	}
    }
	...
	for (j=ArrDims[1]-ugh[1][1]; j<ArrDims[1]; j++)
	  for (i=0; i<LocDims[0]; i++) {
	    idx = ((k+ugh[2][0])*x1len + j)*x0len + i+ugh[0][0];
	    idxbc = i*LocDims[2] + k;
	    udata[idx] = EBdryVals[1][1][idxbc]/E1Units;
	  }
    //   Neumann
	...
	for (i=0; i<LocDims[0]; i++) {
	  ...
	  idx2 = ((k+ugh[2][0])*x1len + j2+ugh[1][0])*x0len + i+ugh[0][0];
	  idxbc = i*LocDims[2] + k;
	  udata[idx] = udata[idx2] + dya*EBdryVals[1][1][idxbc]/E1Units;
	}
    }
  }  // end if rank > 1
  ...
  if (rank > 2) {
	...
	for (j=0; j<LocDims[1]; j++)
	  for (i=0; i<LocDims[0]; i++) {
	    ...
	    idxbc = j*LocDims[0] + i;
	    udata[idx] = EBdryVals[2][0][idxbc]/E1Units;
	  }
    //   Neumann
	...
	for (i=0; i<LocDims[0]; i++) {
	  ...
	  idxbc = j*LocDims[0] + i;
	  udata[idx] = udata[idx2] + dza*EBdryVals[2][0][idxbc]/E1Units;
	}
    }
	...
	for (j=0; j<LocDims[1]; j++)
	  for (i=0; i<LocDims[0]; i++) {
	    idx = (k*x1len + j+ugh[1][0])*x0len + i+ugh[0][0];
	    idxbc = j*LocDims[0] + i;
	    udata[idx] = EBdryVals[2][1][idxbc]/E1Units;
	  }
    //   Neumann
	...
	for (i=0; i<LocDims[0]; i++) {
	  ...
	  idx2 = ((k2+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i+ugh[0][0];
	  idxbc = j*LocDims[0] + i;
	  udata[idx] = udata[idx2] + dza*EBdryVals[2][1][idxbc]/E1Units;
	}
    }
  }  // end if rank > 2
  ...
  if (OnBdry[0][0] && (BdryType[0][0]==1)) 
	...
	for (i=0; i<ugh[0][0]; i++) {
	  idxbc = k*LocDims[1] + j;
	  idx = ((k+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i;
	  udata[idx] = EBdryVals[0][0][idxbc]/E2Units;
	}
  //   Neumann
  if (OnBdry[0][0] && (BdryType[0][0]==2)) {
    i = -1;  i2 = i+1;
	...
	idx2 = ((k+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i2+ugh[0][0];
	idxbc = k*LocDims[1] + j;
	udata[idx] = udata[idx2] + dxa*EBdryVals[0][0][idxbc]/E2Units;
      }
  }
  ...
  if (OnBdry[0][1] && (BdryType[0][1]==1))
	...
	for (i=ArrDims[0]-ugh[0][1]; i<ArrDims[0]; i++) {
	  idxbc = k*LocDims[1] + j;
	  idx = ((k+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i;
	  udata[idx] = EBdryVals[0][1][idxbc]/E2Units;
	}
  //   Neumann
  if (OnBdry[0][1] && (BdryType[0][1]==2)) {
    i = LocDims[0];  i2 = i-1;
	...
	idx2 = ((k+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i2+ugh[0][0];
	idxbc = k*LocDims[1] + j;
	udata[idx] = udata[idx2] + dxa*EBdryVals[0][1][idxbc]/E2Units;
      }
  }
  ...
  if (rank > 1) {
	...
	for (j=0; j<ugh[1][0]; j++)
	  for (i=0; i<LocDims[0]; i++) {
	    idx = ((k+ugh[2][0])*x1len + j)*x0len + i+ugh[0][0];
	    idxbc = i*LocDims[2] + k;
	    udata[idx] = EBdryVals[1][0][idxbc]/E2Units;
	  }
    //   Neumann
    if (OnBdry[1][0] && (BdryType[1][0]==2)) {
      j = -1;  j2 = j+1;
	...
	for (i=0; i<LocDims[0]; i++) {
	  ...
	  idx2 = ((k+ugh[2][0])*x1len + j2+ugh[1][0])*x0len + i+ugh[0][0];
	  idxbc = i*LocDims[2] + k;
	  udata[idx] = udata[idx2] + dya*EBdryVals[1][0][idxbc]/E2Units;
	}
    }
	...
	for (j=ArrDims[1]-ugh[1][1]; j<ArrDims[1]; j++)
	  for (i=0; i<LocDims[0]; i++) {
	    idx = ((k+ugh[2][0])*x1len + j)*x0len + i+ugh[0][0];
	    idxbc = i*LocDims[2] + k;
	    udata[idx] = EBdryVals[1][1][idxbc]/E2Units;
	  }
    //   Neumann
    if (OnBdry[1][1] && (BdryType[1][1]==2)) {
      j = LocDims[1];  j2 = j-1;
	...
	for (i=0; i<LocDims[0]; i++) {
	  ...
	  idx2 = ((k+ugh[2][0])*x1len + j2+ugh[1][0])*x0len + i+ugh[0][0];
	  idxbc = i*LocDims[2] + k;
	  udata[idx] = udata[idx2] + dya*EBdryVals[1][1][idxbc]/E2Units;
	}
    }
  }  // end if rank > 1
  ...
  if (rank > 2) {
	...
	for (j=0; j<LocDims[1]; j++)
	  for (i=0; i<LocDims[0]; i++) {
	    idx = (k*x1len + j+ugh[1][0])*x0len + i+ugh[0][0];
	    idxbc = j*LocDims[0] + i;
	    udata[idx] = EBdryVals[2][0][idxbc]/E2Units;
	  }
    //   Neumann
    if (OnBdry[2][0] && (BdryType[2][0]==2)) {
      k = -1;  k2 = k+1;
	...
	for (i=0; i<LocDims[0]; i++) {
	  ...
	  idx2 = ((k2+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i+ugh[0][0];
	  idxbc = j*LocDims[0] + i;
	  udata[idx] = udata[idx2] + dza*EBdryVals[2][0][idxbc]/E2Units;
	}
    }
	...
	for (j=0; j<LocDims[1]; j++)
	  for (i=0; i<LocDims[0]; i++) {
	    idx = (k*x1len + j+ugh[1][0])*x0len + i+ugh[0][0];
	    idxbc = j*LocDims[0] + i;
	    udata[idx] = EBdryVals[2][1][idxbc]/E2Units;
	  }
    //   Neumann
    if (OnBdry[2][1] && (BdryType[2][1]==2)) {
      k = LocDims[2];  k2 = k-1;
	...
	for (i=0; i<LocDims[0]; i++) {
	  ...
	  idx2 = ((k2+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i+ugh[0][0];
	  idxbc = j*LocDims[0] + i;
	  udata[idx] = udata[idx2] + dza*EBdryVals[2][1][idxbc]/E2Units;
	}
    }
  }  // end if rank > 2
  ...
  if (OnBdry[0][0] && (BdryType[0][0]==1)) 
	...
	for (i=0; i<ugh[0][0]; i++) {
	  idxbc = k*LocDims[1] + j;
	  idx = ((k+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i;
	  udata[idx] = EBdryVals[0][0][idxbc]/E3Units;
	}
  //   Neumann
  if (OnBdry[0][0] && (BdryType[0][0]==2)) {
    i = -1;  i2 = i+1;
	...
	idx2 = ((k+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i2+ugh[0][0];
	idxbc = k*LocDims[1] + j;
	udata[idx] = udata[idx2] + dxa*EBdryVals[0][0][idxbc]/E3Units;
      }
  }
  ...
  if (OnBdry[0][1] && (BdryType[0][1]==1)) 
	...
	for (i=ArrDims[0]-ugh[0][1]; i<ArrDims[0]; i++) {
	  idxbc = k*LocDims[1] + j;
	  idx = ((k+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i;
	  udata[idx] = EBdryVals[0][1][idxbc]/E3Units;
	}
  //   Neumann
  if (OnBdry[0][1] && (BdryType[0][1]==2)) {
    i = LocDims[0];  i2 = i-1;
	...
	idx2 = ((k+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i2+ugh[0][0];
	idxbc = k*LocDims[1] + j;
	udata[idx] = udata[idx2] + dxa*EBdryVals[0][1][idxbc]/E3Units;
      }
  }
  ...
  if (rank > 1) {
	...
	for (j=0; j<ugh[1][0]; j++)
	  for (i=0; i<LocDims[0]; i++) {
	    idx = ((k+ugh[2][0])*x1len + j)*x0len + i+ugh[0][0];
	    idxbc = i*LocDims[2] + k;
	    udata[idx] = EBdryVals[1][0][idxbc]/E3Units;
	  }
    //   Neumann
    if (OnBdry[1][0] && (BdryType[1][0]==2)) {
      j = -1;  j2 = j+1;
	...
	for (i=0; i<LocDims[0]; i++) {
	  ...
	  idx2 = ((k+ugh[2][0])*x1len + j2+ugh[1][0])*x0len + i+ugh[0][0];
	  idxbc = i*LocDims[2] + k;
	  udata[idx] = udata[idx2] + dya*EBdryVals[1][0][idxbc]/E3Units;
	}
    }
	...
	for (j=ArrDims[1]-ugh[1][1]; j<ArrDims[1]; j++)
	  for (i=0; i<LocDims[0]; i++) {
	    idx = ((k+ugh[2][0])*x1len + j)*x0len + i+ugh[0][0];
	    idxbc = i*LocDims[2] + k;
	    udata[idx] = EBdryVals[1][1][idxbc]/E3Units;
	  }
    //   Neumann
    if (OnBdry[1][1] && (BdryType[1][1]==2)) {
      j = LocDims[1];  j2 = j-1;
	...
	for (i=0; i<LocDims[0]; i++) {
	  ...
	  idx2 = ((k+ugh[2][0])*x1len + j2+ugh[1][0])*x0len + i+ugh[0][0];
	  idxbc = i*LocDims[2] + k;
	  udata[idx] = udata[idx2] + dya*EBdryVals[1][1][idxbc]/E3Units;
	}
    }
  }  // end if rank > 1
  ...
  if (rank > 2) {
	...
	for (j=0; j<LocDims[1]; j++)
	  for (i=0; i<LocDims[0]; i++) {
	    idx = (k*x1len + j+ugh[1][0])*x0len + i+ugh[0][0];
	    idxbc = j*LocDims[0] + i;
	    udata[idx] = EBdryVals[2][0][idxbc]/E3Units;
	  }
    //   Neumann
    if (OnBdry[2][0] && (BdryType[2][0]==2)) {
      k = -1;  k2 = k+1;
	...
	for (i=0; i<LocDims[0]; i++) {
	  ...
	  idx2 = ((k2+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i+ugh[0][0];
	  idxbc = j*LocDims[0] + i;
	  udata[idx] = udata[idx2] + dza*EBdryVals[2][0][idxbc]/E3Units;
	}
    }
	...
	for (j=0; j<LocDims[1]; j++)
	  for (i=0; i<LocDims[0]; i++) {
	    idx = (k*x1len + j+ugh[1][0])*x0len + i+ugh[0][0];
	    idxbc = j*LocDims[0] + i;
	    udata[idx] = EBdryVals[2][1][idxbc]/E3Units;
	  }
    //   Neumann
    if (OnBdry[2][1] && (BdryType[2][1]==2)) {
      k = LocDims[2];  k2 = k-1;
	...
	for (i=0; i<LocDims[0]; i++) {
	  ...
	  idx2 = ((k2+ugh[2][0])*x1len + j+ugh[1][0])*x0len + i+ugh[0][0];
	  idxbc = j*LocDims[0] + i;
	  udata[idx] = udata[idx2] + dza*EBdryVals[2][1][idxbc]/E3Units;
	}
    }
  }  // end if rank > 2
  
//   if (debug)
//     printf("Exiting MFSplit::EnforceBoundary routine\n");
