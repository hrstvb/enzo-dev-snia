...
int ExtraOutput(int output_flag, LevelHierarchyEntry *LevelArray[], TopGridData *MetaData, int level,
...
int GenerateGridArray(LevelHierarchyEntry *LevelArray[], int level, HierarchyEntry **Grids[]);
int WriteStreamData(LevelHierarchyEntry *LevelArray[], int level, TopGridData *MetaData, int *CycleCount, int open =
FALSE);
...
#ifdef FAST_SIB
int SetBoundaryConditions(HierarchyEntry *Grids[], int NumberOfGrids, SiblingGridList SiblingList[],
int level, TopGridData *MetaData, ExternalBoundary *Exterior, LevelHierarchyEntry * Level);
#else
...
#ifdef SAB
#ifdef FAST_SIB
int SetAccelerationBoundary(HierarchyEntry *Grids[], int NumberOfGrids, SiblingGridList SiblingList[],
int level, TopGridData *MetaData, ExternalBoundary *Exterior, LevelHierarchyEntry * Level,
int CycleNumber);
#else
...
int NumberOfSubgrids[], fluxes **SubgridFluxesEstimate[], LevelHierarchyEntry *SUBlingList[], TopGridData *MetaData);
...
int NumberOfGrids, int NumberOfSubgrids[]);
int RadiationFieldUpdate(LevelHierarchyEntry *LevelArray[], int level, TopGridData *MetaData);
...
#ifdef FAST_SIB
int CreateSUBlingList(TopGridData *MetaData, LevelHierarchyEntry *LevelArray[], int level,
	SiblingGridList SiblingList[], LevelHierarchyEntry ***SUBlingList);
#else
...
int AdjustRefineRegion(LevelHierarchyEntry *LevelArray[], TopGridData *MetaData, int EL_level);
int AdjustMustRefineParticlesRefineToLevel(TopGridData *MetaData, int EL_level);

#ifdef TRANSFER
int EvolvePhotons(TopGridData *MetaData, LevelHierarchyEntry *LevelArray[], Star *&AllStars, FLOAT GridTime, int level,
int LoopTime = TRUE);
int RadiativeTransferPrepare(LevelHierarchyEntry *LevelArray[], int level, TopGridData *MetaData, Star *&AllStars,
float dtLevelAbove);
int RadiativeTransferCallFLD(LevelHierarchyEntry *LevelArray[], int level, TopGridData *MetaData, Star *AllStars,
	ImplicitProblemABC *ImplicitSolver);
#endif
	...
	, FLOAT dt0, SiblingGridList *SiblingGridListStorage[])
{
	...
	if(dbx)
		fprintf(stderr, "EL: Initialize FSL \n");
	...
	{
#ifdef FAST_SIB
		if(SetBoundaryConditions(Grids, NumberOfGrids, SiblingList, level, MetaData, Exterior,
									LevelArray[level]) == FAIL)
			ENZO_FAIL("Error in SetBoundaryConditions (FastSib)");
#else
		if(SetBoundaryConditions(Grids, NumberOfGrids, level, MetaData, Exterior, LevelArray[level]) == FAIL)
	...
	if(CheckpointRestart == TRUE)
	{
		...
		{
			if(Grids[grid1]->GridData->FillFluxesFromStorage(&NumberOfSubgrids[grid1], &SubgridFluxesEstimate[grid1])
					!= -1)
			{
	...
	while((CheckpointRestart == TRUE) || (dtThisLevelSoFar[level] < dtLevelAbove))
	{
		if(CheckpointRestart == FALSE)
		{

			TIMER_START(level_name);
			SetLevelTimeStep(Grids, NumberOfGrids, level, &dtThisLevelSoFar[level], &dtThisLevel[level], dtLevelAbove);
			...
			if((STARMAKE_METHOD(H2REG_STAR)) && (level == 0) && (StarFormationOncePerRootGridTimeStep))
			{
			...
			Star *AllStars = NULL;
			StarParticleInitialize(Grids, MetaData, NumberOfGrids, LevelArray, level, AllStars,
			...
			RadiativeTransferCallFLD(LevelArray, level, MetaData, AllStars, ImplicitSolver);
			...
			ComputeRandomForcingNormalization(LevelArray, 0, MetaData, &norm, &TopGridTimeStep);
			...
			for(grid1 = 0; grid1 < NumberOfGrids; grid1++)
			{
				...
				CallProblemSpecificRoutines(MetaData, Grids[grid1], grid1, &norm, TopGridTimeStep, level,
											LevelCycleCount);
			...
			SetAccelerationBoundary(Grids, NumberOfGrids, SiblingList, level, MetaData, Exterior, LevelArray[level],
				...
				if(UseHydro)
				{
					...
					else
					{
						if(HydroMethod == HD_RK)
							Grids[grid1]->GridData->RungeKutta2_1stStep(SubgridFluxesEstimate[grid1],
																		NumberOfSubgrids[grid1], level, Exterior,
																		MetaData);
						else if(HydroMethod == MHD_RK)
						{
							Grids[grid1]->GridData->MHDRK2_1stStep(SubgridFluxesEstimate[grid1],
																	NumberOfSubgrids[grid1], level, Exterior, MetaData);
						}
					} //hydro method
				} //use hydro
			} //grids
        ...
        SetBoundaryConditions(Grids, NumberOfGrids, level, MetaData, Exterior, LevelArray[level]);
            ...
            PrepareDensityField(LevelArray, level, MetaData, When);
...
#ifdef SAB
					//Ensure the consistency of the AccelerationField
					SetAccelerationBoundary(Grids, NumberOfGrids, SiblingList, level, MetaData, Exterior,
											LevelArray[level], LevelCycleCount[level]);
				...
				for(grid1 = 0; grid1 < NumberOfGrids; grid1++)
				{
					...
					{
						if(HydroMethod == HD_RK)
							Grids[grid1]->GridData->RungeKutta2_2ndStep(SubgridFluxesEstimate[grid1],
																		NumberOfSubgrids[grid1], level, Exterior,
																		MetaData);
						...
						else if(HydroMethod == MHD_RK)
						{
							...
							Grids[grid1]->GridData->MHDRK2_2ndStep(SubgridFluxesEstimate[grid1],
																	NumberOfSubgrids[grid1], level, Exterior, MetaData);
			...
			for(grid1 = 0; grid1 < NumberOfGrids; grid1++)
			{
				...
				Grids[grid1]->GridData->StarParticleHandler(Grids[grid1]->NextGridNextLevel, level, dtLevelAbove,
															TopGridTimeStep);
...
#ifndef SAB
				if((level != MaximumGravityRefinementLevel || MaximumGravityRefinementLevel == MaximumRefinementLevel)
						&& !PressureFree)
			...
			StarParticleFinalize(Grids, MetaData, NumberOfGrids, LevelArray, level, AllStars,
									TotalStarParticleCountPrevious, OutputNow);
    ...
    SetBoundaryConditions(Grids, NumberOfGrids, level, MetaData,
			...
			{											//[BH]
				if(debug1)
					printf("Grid::EvolveLevel: Calc burning in %d grids\n", NumberOfGrids);		//[BH]
...
//LevelArray[0]->GridData->ExtraFunction("[BH]:>>>>>>>> SolveMHDLi BEFORE diffuse", 0, NULL, NULL); //[BH]EF
					if(debug1)
						printf("Grid::EvolveLevel: Before burning\n");		//[BH]
...
//LevelArray[0]->GridData->ExtraFunction("[BH]:>>>>>>>> SolveMHDLi AFTER diffuse", 0, NULL, NULL); //[BH]EF
					if(debug1)
						printf("Grid::EvolveLevel: After DiffuseBurningFraction\n");	//[BH]
					// Repeat SetBoundaryConditions after burning.				//[BH]
				}		//end for gridr1								//[BH]

				if(CallSetBoundaryConditionsAfterBurning)
				{				//[BH]
					if(debug1)
						printf("Grid::EvolveLevel: Before 2nd SetBoundaryConditions\n");	//[BH]
#ifdef FAST_SIB										//[BH]
...
#endif											//[BH]
					if(debug1)
						printf("Grid::EvolveLevel: After 2nd SetBoundaryConditions\n");	//[BH]
					EXTRA_OUTPUT_MACRO(26, "After 2nd SBC")
					//[BH]
				} //end if call SBC							//[BH]
				else										//[BH]
				{										//[BH]
					if(debug1)
						printf("Grid::EvolveLevel: Skip 2nd SetBoundaryConditions\n");	//[BH]
				}	//end else dont call SBC

				if(debug1)
					printf("Grid::EvolveLevel: Burning done\n");			//[BH]
			}			// end if use burning								//[BH]
			else										//[BH]
			{											//[BH]
				if(debug1)
					printf("Grid::EvolveLevel: Skip burning\n");				//[BH]
...
#endif
							,
							dt0, SiblingGridListStorage) == FAIL)
			{
...
#ifdef FAST_SIB
		CreateSUBlingList(MetaData, LevelArray, level, SiblingList, &SUBlingList);
#else
    CreateSUBlingList(MetaData, LevelArray, level, &SUBlingList);
		...
		UpdateFromFinerGrids(level, Grids, NumberOfGrids, NumberOfSubgrids, SubgridFluxesEstimate, SUBlingList,
								MetaData);
...
//     } // if WritePotential
		...
		for(grid1 = 0; grid1 < NumberOfGrids; grid1++)
		{
			Grids[grid1]->GridData->CollectGridInformation(GridMemory, GridVolume, NumberOfCells, AxialRatio,
															CellsTotal, Particles);
		...
		for(grid1 = 0; grid1 < NumberOfGrids; grid1++)
			if(Grids[grid1]->GridData->ComputeEnergyGrowthTimeStep() == FAIL)
	...
	if(debug)
		fprintf(stdout, "EvolveLevel[%"ISYM"]: NumberOfSubCycles = %"ISYM" (%"ISYM" total, %"ISYM" sub)\n", level,
				cycle, LevelCycleCount[level], LevelSubCycleCount[level]);
