...
int IncludeCRs)
{
	...
	{
		if(this->IdentifyPhysicalQuantities(DensNum, GENum, Vel1Num, Vel2Num, Vel3Num, TENum, B1Num, B2Num,
											B3Num) == FAIL)
		{
			ENZO_FAIL("Error in IdentifyPhysicalQuantities.\n");
		}
	...
	if(time == Time)
		...
		for(i = 0; i < size; i++)
		{
			...
			else
			{
				...
				if(EOSType > 0)
				{
					...
					/* also reset energy */
					float kineticE = OneHalf * (velocity1 * velocity1 + velocity2 * velocity2 + velocity3 * velocity3);
				...
				else
				{
					if(DualEnergyFormalism == 0)
					{
						...
						gas_energy = total_energy
								- OneHalf * (velocity1 * velocity1 + velocity2 * velocity2 + velocity3 * velocity3);
					...
					min_pressure = MinimumSupportEnergyCoefficient * (Gamma - 1.0) * density * density;
		...
		for(i = 0; i < size; i++)
		{
			...
			if(EquationOfState == 1)
			{
				pressure[i] = coef * BaryonField[DensNum][i] + coefold * OldBaryonField[DensNum][i];
			...
			else
			{
				...
				total_energy = coef * BaryonField[TENum][i] + coefold * OldBaryonField[TENum][i];
				...
				else
				{
					...
					{
						gas_energy = coef * BaryonField[GENum][i] + coefold * OldBaryonField[GENum][i];
					}
	...
	if(MultiSpecies > 1)
	{
		...
		float TemperatureUnits = 1, number_density, nH2, GammaH2Inverse, GammaInverse = 1.0 / (Gamma - 1.0), x, Gamma1,
				temp;
		...
		if(IdentifySpeciesFields(DeNum, HINum, HIINum, HeINum, HeIINum, HeIIINum, HMNum, H2INum, H2IINum, DINum, DIINum,
									HDINum) == FAIL)
		{
		...
		for(i = 0; i < size; i++)
		{
			...
			number_density = 0.25 * (BaryonField[HeINum][i] + BaryonField[HeIINum][i] + BaryonField[HeIIINum][i])
			...
			Gamma1 = 1.0 + (nH2 + number_density) / (nH2 * GammaH2Inverse + number_density * GammaInverse);
