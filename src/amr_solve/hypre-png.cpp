...
void images_allocate(double *images3[3],
		     ...
		     int     top_level,
		     int     scale);
void images_grid_add(AMRsolve_Grid   * grid,
		     double * images3[3],
		     int      n3[3],
		     int      ni3[3],
		     int      nl3[3],
...
int main(int argc, char **argv)
{
  ...
  typedef std::vector<AMRsolve_Grid *> gridlist;
  gridlist levels[MAX_LEVELS];
  for (; iarg<argc; iarg++) {
    printf ("Reading %s\n",argv[iarg]);
    FILE * fp = fopen(argv[iarg],"r");
    AMRsolve_Grid * grid = new AMRsolve_Grid("u",fp);
  ...
  for (l=0; l<MAX_LEVELS; l++) {
    for (j=0; j<levels[l].size(); j++) {
      ...
      int kg = int(pow(2.0,top_level - levels[l][j]->level())); 
		     ...
		     int      scale)
{
  ...
  int kg = int(pow(2.0,top_level - grid->level())); 
